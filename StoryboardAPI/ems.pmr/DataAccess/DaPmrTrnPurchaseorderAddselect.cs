...
{
    public class DaPmrTrnPurchaseorderAddselect
    {
        ...
        string msEmployeeGID, lsemployee_gid, lsentity_code, lsdesignation_code, lsCode, lstaxpercentage, msPOGetGID, msPO1GetGID, msGetGID, lsreceived, lsReference_gid, lstPR_PO_flag,
            ...
            mspGetGID, msGetGid, msGetGid1, msGetGID1, msGetGID21, msGetPrivilege_gid, msGetModule2employee_gid;
        ...
        public void DaGetPurchaseOrderAddselect(string branch_gid, MdlPmrTrnPurchaseorderAddselect values)
        {
            try
            {
        ...
        public void DaGetPurchaseOrderSelectSummary(string purchaserequisition_gid, MdlPmrTrnPurchaseorderAddselect values)
        {
            try
            {
                ...
                msSQL = "   select a.purchaserequisitiondtl_gid,a. purchaserequisition_gid ,a. " +
                        " product_gid, a.product_code,a. product_name, d.productuom_gid,a. productuom_name," +
                        " a.productgroup_gid,a. productgroup_name, a.qty_requested, a. seq_no, " +
                        " a.user_gid, a. display_field,a.  pr_originalqty,a. pr_newproductstatus," +
                        " a.product_price  from  pmr_trn_tpurchaserequisitiondtl   a " +
                        ...
                        " LEFT JOIN adm_mst_tuser y ON y.user_gid = a.created_by" +
                ...
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow dt in dt_datatable.Rows)
                    {
                        getModuleList.Add(new GetPurchaseOrder1
                        {
                            ...
                            qty_ordered = dt["pr_originalqty"].ToString(),
        ...
        public void DaPostPoaddSubmit(string user_gid, string purchaserequisition_gid, MdlPmrTrnPurchaseorderAddselect values)
        {
            try
            {
        ...
        public void DaGetPurchaseOrderSelect(string user_gid, MdlPmrTrnPurchaseorderAddselect values)
        {
            try
            {
        ...
        public void DaGetRaisePOSummary(string purchaserequisition_gid, MdlPmrTrnPurchaseorderAddselect values)
        {
            try
            {
        ...
        public void DaGetProductsearchSummary1(string producttype_gid, string product_gid, string vendor_gid, MdlPmrTrnPurchaseorderAddselect values)
        {
            try
            {
        ...
        public void DaPostOverallSubmit(string user_gid, postpo_list values)
        {
...

            {
                if (string.IsNullOrEmpty(values.poref_no))
                {
                    ...
                    msGetGID21 = objcmnfunctions.getSequencecustomizerGID("PPOP", "PMR", values.branch_name);
                    foreach (var data in values.Posummary_list)
                        ...
                        //int discount = (discountAmount * discount_percentage)/100;
                            msSQL = " insert into pmr_trn_tpurchaseorderdtl (" +
                                    ...
                                    "'" + data.product_gid + "', " +
                                    "'" + data.product_code + "',";
                                    if (!string.IsNullOrEmpty(data.product_name) && data.product_name.Contains("'"))
                                    {
                                        msSQL += "'" + data.product_name.Replace("'", "\\\'") + "',";
                                    }
                                    else
                                    {
                                        msSQL += "'" + data.product_name + "', ";
                                    }
                                    if (!string.IsNullOrEmpty(data.productuom_name) && data.productuom_name.Contains("'"))
                                    {
                                        msSQL += "'" + data.productuom_name.Replace("'", "\\\'") + "',";
                                    }
                                    else
                                    {
                                        msSQL += "'" + data.productuom_name + "', ";
                                    }
                                    msSQL +=    "'" + data.productuom_gid + "', " +
                    ...
                    msSQL = " insert into pmr_trn_tpurchaseorder (" +
                        ...
                        if (!string.IsNullOrEmpty(values.despatch_mode) && values.despatch_mode.Contains("'"))
                        {
                            msSQL += "'" + values.despatch_mode.Replace("'", "\\\'") + "',";
                        }
                        else
                        {
                            msSQL += "'" + values.despatch_mode + "', ";
                        }
        ...
        public void DaPostPurchaseOrderfileupload(HttpRequest httpRequest, result objResult, string user_gid)
        {
            try
            {
                ...
                var renewaldate = httpRequest.Form["renewal_date"].ToString();
                var frequencyterms = httpRequest.Form["frequency_terms"].ToString();
                var renewalmode = httpRequest.Form["renewal_mode"].ToString();
                ...
                if (string.IsNullOrEmpty(poNo))
                {
                    ...
                    msSQL = " insert into pmr_trn_tpurchaseorder (" +
                        ...
                        {
                            msSQL += "'" + vendorCompanyName.Replace("'", "\\\'") + "', ";
                        }
                        else
                        {
                            msSQL += "'" + vendorCompanyName + "', ";
                        }
                    ...
                    {
                        msSQL += "'" + paymentTerms.Replace("'", "\\\'") + "',";
                    }
                    else
                    {
                        msSQL += "'" + paymentTerms + "', ";
                    }
                    msSQL += "'" + requestorDetails + "',";
        ...
        public void DaGetPurchaseOrderSelectSummarytax(string user_gid, string purchaserequisition_gid, string vendor_gid, MdlPmrTrnPurchaseorderAddselect values)
       {
            try
            {
                                ...
                                if (i == 0)
                                {
                                    msSQL = " insert into pmr_tmp_tpurchaseorder (" +
                                            ...
                                            //"'" + dta["product_name"].ToString() + "'," +
                                             if (!string.IsNullOrEmpty(dta["product_name"].ToString()) && dta["product_name"].ToString().Contains("'"))
                                              {
                                               msSQL += "'" + dta["product_name"].ToString().Replace("'", "\\\'") + "',";
                                              }
                                            else
                                            {
                                                msSQL += "'" + dta["product_name"].ToString() + "', ";
                                            }
                                    msSQL += "'" + dta["productuom_name"].ToString() + "'," +
                                            "'" + dta["productuom_gid"].ToString() + "'," +
                                            "'" + dta["qty_requested"].ToString() + "', " +
                                            "'" + dt["tax_prefix"].ToString() + "', " +
                                            "'" + dt["tax_percentage"].ToString() + "', " +
                                            "'" + dt["tax_gid"].ToString() + "', " +
                                            "'" + dta["purchaserequisition_gid"].ToString() + "', " +
                                            "'" + dta["productgroup_name"].ToString() + "', " +
                                            "'" + user_gid + "', ";
                                            //"'" + dta["display_field"].ToString() + "', " +
                                      if (!string.IsNullOrEmpty(dta["display_field"].ToString()) && dta["display_field"].ToString().Contains("'"))
                                    {
                                        msSQL += "'" + dta["display_field"].ToString().Replace("'", "\\\'") + "',";
                                    }
                                    else
                                    {
                                        msSQL += "'" + dta["display_field"].ToString() + "', ";
                                    }
                                    msSQL += "'" + dta["purchaserequisitiondtl_gid"].ToString() + "') ";
