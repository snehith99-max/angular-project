...
namespace ems.pmr.DataAccess
{
    public class DaPmrVendor360
    {
        ...
        public void DaGetTilesCount(string vendor_gid, MdlPmrVendor360 values)
        {
            try
            {
                msSQL = "select(select count(a.purchaseorder_gid) from pmr_trn_tpurchaseorder a" +
                    ...
                    " (select format(sum(a.payment_total),2) from acp_trn_tpayment a  left join acp_mst_tvendor b on a.vendor_gid = b.vendor_gid " +
                    " where a.vendor_gid='" + vendor_gid + "') as paymentamount," +
                    " (select format(sum(a.total_amount),2) as total from pmr_trn_tpurchaseorder a   left join acp_mst_tvendor b on a.vendor_gid = b.vendor_gid " +
                    ...
                    " (select format(sum(a.invoice_amount),2) from acp_trn_tinvoice a  left join acp_mst_tvendor b on a.vendor_gid = b.vendor_gid" +
                    ...
                    " (select format(sum(a.total_amount),2) from pmr_trn_tquotation a " +
                    " left join acp_mst_tvendor b on a.vendor_gid = b.vendor_gid " +
        ...
        public void DaGetVendorDetails(string vendor_gid, MdlPmrVendor360 values)
        {
        ...
        public void DaGetPurchasetatus(string vendor_gid, MdlPmrVendor360 values)
        {
            try
            {
        ...
        public void DaGetPaymentCount(string vendor_gid, MdlPmrVendor360 values)
        {
            try
            {
