...
namespace ems.pmr.Controllers
{
    ...
    public class PmrMstPurchaseConfigController : ApiController
    {
        ...
        [ActionName("UpdateContractPOConfig")]
        [HttpPost]
        public HttpResponseMessage UpdateContractPOConfig(salesconfiglist values)
        {
            string token = Request.Headers.GetValues("Authorization").FirstOrDefault();
            getsessionvalues = Objgetgid.gettokenvalues(token);
            objDaSmrMstSalesConfig.DaUpdateContractPOConfig(getsessionvalues.employee_gid, values);
            return Request.CreateResponse(HttpStatusCode.OK, values);
        }
