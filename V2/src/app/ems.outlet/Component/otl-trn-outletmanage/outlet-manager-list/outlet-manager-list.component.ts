...
@Component({
	selector: 'outletmanager-list',
	templateUrl: './outlet-manager-list.component.html',
	styleUrls: ['./outlet-manager-list.component.scss']
  })

export class OutletManagerListComponent implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'UnAssigned Managers', remove: 'Assigned Managers', all: 'All Select', none: 'All Unselect', direction: OutletManagerListComponent.LTR, draggable: true };
	...
	@Input() height = '100px';
	@Input() filter1 = false;
	@Input() format1 = OutletManagerListComponent.DEFAULT_FORMAT;
	@Input() sort1 = false;
	@Input()
	...
	available: OutletList;
	confirmed: OutletList;
	sourceDiffer: any;
	destinationDiffer: any;
	private sorter = (a: any, b: any) => { return (a._name < b._name) ? -1 : ((a._name > b._name) ? 1 : 0); };

	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: OtlTrnOutletmanageComponent, private NgxSpinnerService: NgxSpinnerService) {
		this.available = new OutletList(OutletManagerListComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new OutletList(OutletManagerListComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: { [key2: string]: SimpleChange }) {
		...
		if (changeRecord['format1']) {
			...
			if (typeof (this.format1.direction) === 'undefined') {
				this.format1.direction = OutletManagerListComponent.LTR;
			}

			if (typeof (this.format1.add) === 'undefined') {
				this.format1.add = OutletManagerListComponent.DEFAULT_FORMAT.add;
			}

			if (typeof (this.format1.remove) === 'undefined') {
				this.format1.remove = OutletManagerListComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof (this.format1.all) === 'undefined') {
				this.format1.all = OutletManagerListComponent.DEFAULT_FORMAT.all;
			}

			if (typeof (this.format1.none) === 'undefined') {
				this.format1.none = OutletManagerListComponent.DEFAULT_FORMAT.none;
			}

			if (typeof (this.format1.draggable) === 'undefined') {
				this.format1.draggable = OutletManagerListComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	direction() {
		return this.format1.direction === OutletManagerListComponent.LTR;
	}

	dragEnd(list: OutletList) {
		if (list) {
	...
	drag(event: DragEvent, item: any, list: OutletList) {
	...
	allowDrop(event: DragEvent, list: OutletList) {
	...
	drop(event: DragEvent, list: OutletList) {
	...
	private makeUnavailable(source: OutletList, item: any) {
	...
	moveItem(source: OutletList, target: OutletList, item: any = null, trueup = true) {
		let i = 0;
	...
	// btnremove(source: OutletList, target: OutletList, item: any = null, trueup = true) {
	...
	addbtn(source: OutletList, target: OutletList, item: any = null, trueup = true, campaign_gid: string) {
		...
		const campaignUnassignData: ICampaignUnAssign = { campaign_gid: campaign_gid, outletassignmanager: [] };
		if (target.list.length === 0) {
			campaignUnassignData.outletassignmanager = target.sift.map((item: any) => item._id); // Pass all values from the target list
		} else {
			campaignUnassignData.outletassignmanager = target.pick; // Pass selected values only
		}
		this.NgxSpinnerService.show();
		const url = 'OutletManage/PostoutAssignedManagerlist';
	...
	shiftClick(event: MouseEvent, index: number, source: OutletList, item: any) {
	...
	selectAll(source: OutletList) {
		source.pick.length = 0;
		source.pick = source.sift.slice(0);
	}

	selectNone(source: OutletList) {
		source.pick.length = 0;
	}

	isAllSelected(source: OutletList) {
	...
	isAnySelected(source: OutletList) {
	...
	private unpick(source: OutletList) {
