...
export class DualListComponent implements DoCheck, OnChanges {
	...
	moveToConfirmed(): void {
		this.moveItem(this.available, this.confirmed);
		console.log('success')
		console.log(this.confirmed.sift)
	}
	...
	addbtn(source: BasicList, target: BasicList, item: any = null, trueup = true, campaign_gid: string) {
		...
		const campaignUnassignData: ICampaignUnAssign = { campaign_gid:campaign_gid, outcampaignassignemp: [] };
		// Assign values based on source list and pick
		if (source.list.length === 0) {
			campaignUnassignData.outcampaignassignemp = source.sift.map((item: any) => item._id); // Pass all values from the source list
		} else {
			campaignUnassignData.outcampaignassignemp = source.pick; // Pass selected values only
		}
		this.NgxSpinnerService.show();
		const url ='OutletManage/PostOutAssignedEmplist';
