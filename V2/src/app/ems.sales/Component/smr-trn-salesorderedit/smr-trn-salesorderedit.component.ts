...
@Component({
  selector: 'app-smr-trn-salesorderedit',
  templateUrl: './smr-trn-salesorderedit.component.html',
  styleUrls: ['./smr-trn-salesorderedit.component.scss']
})

export class SmrTrnSalesordereditComponent {
  ...
  const options: Options = {
   ...
   const deencryptedParam = AES.decrypt(this.salesordergid, secretKey).toString(enc.Utf8);
   ...
   //this. GetViewsalesorderdetails(deencryptedParam);
    ...
    this.combinedFormData = new FormGroup({
      ...
      frequency_term:new FormControl (''),
    ...
    this.service.get(url).subscribe((result: any) => {
      ...
      const branchName = firstBranch.branch_name;
  ...
  //   var url = 'SmrTrnSalesorder/GetOnChangeShipping'
  //   let params = {
  //     customer_gid:customer_gid
  //   }
  //   this.service.getparams(url,params).subscribe((apiresponse: any) => {
  //     this.shippingto_list = apiresponse.shippingto_list;
  //   });
  // }
 
  GetEditSalesOrder(params:any){
    debugger
    var url='SmrTrnSalesorder/GetEditsalesorderSummary'
    let param = {
    ...
    this.shippingto_list = result.shippingto_list;
    ...
    this.combinedFormData.get("salesorder_gid")?.setValue(params);

    this.combinedFormData.get("so_referencenumber")?.setValue(this.salesorderedit_list[0].so_referenceno1);
    this.combinedFormData.get("customer_name")?.setValue(this.salesorderedit_list[0].customer_gid);
   ...
   this.txtcustomercontact_gid =  this.salesorderedit_list[0].customercontact_gid
  ...
  //   var url='SmrTrnSalesorder/Getsalesorderdtlrecords'
  ...
  onFileChange(event: any) {
    ...
    //     const image =  this.existingFile?.split('.net/');
    //       const page = image[0];
    //      const url = page.split('?');
    //       const imageurl = url[0];
    //       const parts = imageurl.split('.');
    //       const extension = parts.pop();
    //     var params = {fileurl:this.existingFileUrl,filename:this.existingFileName}
  ...
  onAnchorClick(event:any){
    if(this.existingFileUrl !=null && this.existingFileUrl != ""){
      const file_name = this.existingFileName;
      const image = this.existingFileUrl.split('.net/');
    const page = image[0];
   const url = page.split('?');
    const imageurl = url[0];
    const parts = imageurl.split('.');
    ...
    this.ToastrService.warning("No file has been uploaded for this SO");
  }
  }
  removeFile() {
  ...
  onUpdate() {
   ...
   formData.append("salesorder_gid",deencryptedParam);
   formData.append("customer_gid",this.combinedFormData.value.customer_name);
   ...
   formData.append("frequency_term",this.combinedFormData.value.frequency_term);
   ...
   formData.append("shipping_to",this.combinedFormData.value.shipping_address);
   ...
   formData.append("tax_gid",this.combinedFormData.value.tax_name4);
   ...
   var url = 'SmrTrnSalesorder/UpdateSalesOrderfileupload'
    this.NgxSpinnerService.show();
   ...
   const deencryptedParam = AES.decrypt(this.salesordergid, secretKey).toString(enc.Utf8);
    var params = {
      ...
      frequency_term: this.combinedFormData.value.frequency_term,
      ...
      shipping_to: this.combinedFormData.value.shipping_address,
      ...
      tax_gid: this.combinedFormData.value.tax_name4,
    ...
    var url = 'SmrTrnSalesorder/UpdateSalesOrder'
    this.NgxSpinnerService.show();
  ...
  prodcutaddtotalcal() {
    ...
    const customer = this.combinedFormData.value.customer_name;
