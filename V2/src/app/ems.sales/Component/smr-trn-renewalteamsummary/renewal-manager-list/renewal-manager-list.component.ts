...
@Component({
  selector: 'app-renewal-manager-list',
  templateUrl: './renewal-manager-list.component.html',
  styleUrls: ['./renewal-manager-list.component.scss']
})

export class RenewalManagerListComponent implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'UnAssigned Managers', remove: 'Assigned Managers', all: 'All Select', none: 'All Unselect', direction: RenewalManagerListComponent.LTR, draggable: true };
	...
	@Input() height = '100px';
	@Input() filter1 = false;
	@Input() format1 = RenewalManagerListComponent.DEFAULT_FORMAT;
	@Input() sort1 = false;
	@Input()
	...
	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: SmrTrnRenewalteamsummaryComponent, private NgxSpinnerService: NgxSpinnerService) {
		this.available = new TeamList(RenewalManagerListComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new TeamList(RenewalManagerListComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: { [key2: string]: SimpleChange }) {
		...
		if (changeRecord['format1']) {
			...
			if (typeof (this.format1.direction) === 'undefined') {
				this.format1.direction = RenewalManagerListComponent.LTR;
			}

			if (typeof (this.format1.add) === 'undefined') {
				this.format1.add = RenewalManagerListComponent.DEFAULT_FORMAT.add;
			}

			if (typeof (this.format1.remove) === 'undefined') {
				this.format1.remove = RenewalManagerListComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof (this.format1.all) === 'undefined') {
				this.format1.all = RenewalManagerListComponent.DEFAULT_FORMAT.all;
			}

			if (typeof (this.format1.none) === 'undefined') {
				this.format1.none = RenewalManagerListComponent.DEFAULT_FORMAT.none;
			}

			if (typeof (this.format1.draggable) === 'undefined') {
				this.format1.draggable = RenewalManagerListComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	direction() {
		return this.format1.direction === RenewalManagerListComponent.LTR;
	}
	...
	addbtn(source: TeamList, target: TeamList, item: any = null, trueup = true, campaign_gid: string) {
		const campaignUnassignData: ICampaignUnAssign = { campaign_gid: campaign_gid, campaignunassign: [] };
		if (target.list.length === 0) {
			campaignUnassignData.campaignunassign = target.sift.map((item: any) => item._id); // Pass all values from the target list
		} else {
			campaignUnassignData.campaignunassign = target.pick; // Pass selected values only
		}
		this.NgxSpinnerService.show();
		const url = 'SmrTrnRenewalteamsummary/PostManagerUnassignedlist';
