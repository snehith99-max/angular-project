...
@Component({
  selector: 'app-smr-trn-agreementtoinvoicetag',
  templateUrl: './smr-trn-agreementtoinvoicetag.component.html',
  styleUrls: ['./smr-trn-agreementtoinvoicetag.component.scss']
})
export class SmrTrnAgreementtoinvoicetagComponent {
  ...
  GetMappingSummary(renewal_gid_key1: any) {
    debugger
    var api = 'SmrTrnRenewalsummary/Getagreementtoinvoicetag'
    ...
    this.service.getparams(api, param).subscribe((result: any) => {
      ...
      this.assignproduct_list = this.response_data.invoicetagsummary_list;
      setTimeout(()=>{  
  ...
  assign(){
    debugger;
     ...
     this.pick = this.selection.selected  
      this.CurObj.invoicetagsummary_list = this.pick
        if (this.CurObj.invoicetagsummary_list.length === 0) {
        this.ToastrService.warning("Select atleast one Tag Invoice");
        return;
      } 
      this.NgxSpinnerService.show();
        var url = 'SmrTrnRenewalsummary/PostMappedinvoicetag';  
