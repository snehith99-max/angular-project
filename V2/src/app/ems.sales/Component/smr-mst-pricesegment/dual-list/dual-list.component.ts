...
export class DualListComponent implements DoCheck, OnChanges {
	...
	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: SmrMstPricesegmentComponent, private NgxSpinnerService: NgxSpinnerService) {
	...
	onUpdate() {
		...
		if (this.confirmed.sift.length > 0) {
			this.pricesegment_gid = this.key1
		}
		else {
			this.pricesegment_gid = this.key1
		}
		...
		this.addbtn(this.confirmed, this.available, null, false, this.pricesegment_gid);
	}
	...
	addbtn(source: BasicListComponent, target: BasicListComponent, item: any = null, trueup = true, pricesegment_gid: string) {
		// const campaignAssignData: ICampaignAssign = { pricesegment_gid:pricesegment_gid, campaignassign: [] };
		...
		const campaignUnassignData: ICampaignUnAssign = { pricesegment_gid:pricesegment_gid, campaignunassign: [] };
		// Assign values based on source list and pick
		if (source.list.length === 0) {
			campaignUnassignData.campaignunassign = source.sift.map((item: any) => item._id); // Pass all values from the source list
		} else {
			campaignUnassignData.campaignunassign = source.pick; // Pass selected values only
		}
		this.NgxSpinnerService.show();
		const url ='SmrMstPricesegmentSummary/PostAssignedlist';
	...
	GetTeamSummary() {
		this.NgxSpinnerService.show();
		var api = 'SmrMstPricesegmentSummary/GetSmrMstPricesegmentSummary'
		this.service.get(api).subscribe((result: any) => {
		  this.team_list1 = result.marketingteam_list;
