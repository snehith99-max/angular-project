...
@Component({
  selector: 'app-smr-trn-einvoice',
  templateUrl: './smr-trn-einvoice.component.html',
  styleUrls: ['./smr-trn-einvoice.component.scss']
})
export class SmrTrnEinvoiceComponent {
  ...
  ngOnInit(): void {
    ...
    var api = 'Einvoice/GetEInvoiceProductSummary';
  ...
  constructor(private fb: FormBuilder, private route: ActivatedRoute, private router: Router, private service: SocketService, private ToastrService: ToastrService) {
    this.einvoicedtlform = new FormGroup({
      ...
      transactionDetails_TaxSch: new FormControl(),
  ...
  GetEInvoiceData() {
    ...
    this.service.getparams(api, param).subscribe((result: any) => {
      ...
      this.einvoicedtlform.get("transactionDetails_TaxSch")?.setValue("GST");
      this.einvoicedtlform.get("transactionDetails_IgstOnIntra")?.setValue("N");
      this.einvoicedtlform.get("transactionDetails_RegRev")?.setValue("N");
      this.einvoicedtlform.get("transactionDetails_tranaction_type")?.setValue("Regular");
      this.einvoicedtlform.get("transactionDetails_SupTyp")?.setValue("B2B");
      this.einvoicedtlform.get("documentDetails_Typ")?.setValue("INV");
