...
// @Component({
//   selector: 'app-smr-rpt-receiptreport',
//   templateUrl: './smr-rpt-receiptreport.component.html',
//   styleUrls: ['./smr-rpt-receiptreport.component.scss']
// })
// export class SmrRptReceiptreportComponent {
...
export class SmrRptReceiptreportComponent {
  ...
  GetReceiptForLastSixMonths_List:any;
...
this.maxDate = today.toISOString().split('T')[0];
  ...
  GetInvoiceForLastSixMonths() {
    ...
    var url = 'SmrRptInvoiceReport/GetReceiptForLastSixMonths'
    this.service.get(url).subscribe((result: any) => {
      $('#GetReceiptForLastSixMonths_List').DataTable().destroy();
      this.responsedata = result;
      this.GetReceiptForLastSixMonths_List = this.responsedata.GetReceiptForLastSixMonths_List;
      this.NgxSpinnerService.hide();
      if (this.GetReceiptForLastSixMonths_List == null) {
        this.flag = false;
      }
      const categories = this.GetReceiptForLastSixMonths_List.map((entry: { month: any; }) => entry.month);
      const data = this.GetReceiptForLastSixMonths_List.map((entry: { paymentamount: any; }) => entry.paymentamount);
      this.chartOptions = {
        chart: {
          type: 'bar',
          height: 300,
  ...
  onSearchClick() {
    var url = 'SmrRptInvoiceReport/GetReceiptReportForLastSixMonthsSearch';
    this.NgxSpinnerService.show()
    let params = {
    ...
    this.service.getparams(url, params).subscribe((result: any) => {
      ...
      this.chartOptions.xaxis.categories = this.GetReceiptForLastSixMonths_List.map((entry: { month: any }) => entry.month);
      this.chartOptions.series[0].data = this.GetReceiptForLastSixMonths_List.map((entry: { paymentamount: any }) => entry.paymentamount);
      this.NgxSpinnerService.hide()
      this.renderChart()
    })
  }
  ...
  ondetail(month: any, year: any) {
   ...
   console.log("cejcjning")
    var url = 'SmrRptInvoiceReport/GetReceiptDetailSummary'
    let param = {
      from_date: this.from_date,
      to_date: this.to_date,
      month: month,
    ...
    this.service.getparams(url, param).subscribe((result: any) => {
      this.responsedata = result;
      this.GetInvoiceDetailSummarylist = result.GetReceiptDetailSummary;
  ...
  onRowClick(index: number) {
    ...
    this.GetReceiptForLastSixMonths_List.forEach((data: any, i: number) => {
