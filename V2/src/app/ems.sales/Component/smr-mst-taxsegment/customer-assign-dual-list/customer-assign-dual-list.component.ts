...
@Component({
	selector: 'customer-assign-dual-list',
	styleUrls: ['./customer-assign-dual-list.component.scss'],
	templateUrl: './customer-assign-dual-list.component.html'
})

export class CustomerAssignDualListComponent implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'UnAssigned Customers', remove: 'Assigned Customers', all: 'All Select', none: 'All Unselect', direction: CustomerAssignDualListComponent.LTR, draggable: true };

	@Input() id = `customer-assign-dual-list-${nextId++}`;
	@Input() key = '_id';
	@Input() key1 = '_key1';
	@Input() taxsegment_gid = '_key1';
	@Input() display = '_name';
	@Input() height = '100px';
	@Input() filter = false;
	@Input() format = CustomerAssignDualListComponent.DEFAULT_FORMAT;
	@Input() sort = false;
	@Input()
	...
	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: SmrMstTaxsegmentComponent, private NgxSpinnerService: NgxSpinnerService) {
		this.available = new BasicList(CustomerAssignDualListComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new BasicList(CustomerAssignDualListComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
		...
		if (changeRecord['format']) {
			...
			if (typeof (this.format.direction) === 'undefined') {
				this.format.direction = CustomerAssignDualListComponent.LTR;
			}

			if (typeof (this.format.add) === 'undefined') {
				this.format.add = CustomerAssignDualListComponent.DEFAULT_FORMAT.add;
			}

			if (typeof (this.format.remove) === 'undefined') {
				this.format.remove = CustomerAssignDualListComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof (this.format.all) === 'undefined') {
				this.format.all = CustomerAssignDualListComponent.DEFAULT_FORMAT.all;
			}

			if (typeof (this.format.none) === 'undefined') {
				this.format.none = CustomerAssignDualListComponent.DEFAULT_FORMAT.none;
			}

			if (typeof (this.format.draggable) === 'undefined') {
				this.format.draggable = CustomerAssignDualListComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	direction() {
		return this.format.direction === CustomerAssignDualListComponent.LTR;
	}
	...
	onUpdate() {
		if (this.confirmed.sift.length > 0) {
			this.taxsegment_gid = this.confirmed.sift[0]._key1
		}
		else {
			this.taxsegment_gid = this.available.sift[0]._key1
		}
		this.selectAll(this.available);
		this.selectAll(this.confirmed);
		this.addbtn(this.confirmed, this.available, null, false, this.taxsegment_gid);
	}
	...
	addbtn(source: BasicList, target: BasicList, item: any = null, trueup = true, taxsegment_gid: string) {
		...
		const campaignUnassignData: ICampaignUnAssign = { taxsegment_gid:taxsegment_gid, campaignunassignemp: [] };
		// Assign values based on source list and pick
		if (source.list.length === 0) {
		...
		const url ='SmrMstTaxSegment/PostCustomerUnassignedlist';
	...
	GetTaxSegmentSummary() {
		...
		this.service.get(api).subscribe((result: any) => {
		  ...
		  this.parentApiCall.emit(this.taxsegment_list);
		});
	  }
