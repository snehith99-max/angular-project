...
import { VendorBasicList } from './vendorbasic-list'
...
@Component({
  selector: 'vendor-assign-dual-list',
  templateUrl: './vendor-assign-dual-list.component.html',
  styleUrls: ['./vendor-assign-dual-list.component.scss']
})

export class VendorAssignDualListComponent implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'UnAssigned Vendors', remove: 'Assigned Vendors', all: 'All Select', none: 'All Unselect', direction: VendorAssignDualListComponent.LTR, draggable: true };
	...
	@Input() height = '100px';
	@Input() filter1 = false;
	@Input() format1 = VendorAssignDualListComponent.DEFAULT_FORMAT;
	@Input() sort1 = false;
	@Input()
	...
	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: SmrMstTaxsegmentComponent, private NgxSpinnerService: NgxSpinnerService) {
		this.available = new VendorBasicList(VendorAssignDualListComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new VendorBasicList(VendorAssignDualListComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: { [key2: string]: SimpleChange }) {
		...
		if (changeRecord['format1']) {
			...
			if (typeof (this.format1.direction) === 'undefined') {
				this.format1.direction = VendorAssignDualListComponent.LTR;
			}

			if (typeof (this.format1.add) === 'undefined') {
				this.format1.add = VendorAssignDualListComponent.DEFAULT_FORMAT.add;
			}

			if (typeof (this.format1.remove) === 'undefined') {
				this.format1.remove = VendorAssignDualListComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof (this.format1.all) === 'undefined') {
				this.format1.all = VendorAssignDualListComponent.DEFAULT_FORMAT.all;
			}

			if (typeof (this.format1.none) === 'undefined') {
				this.format1.none = VendorAssignDualListComponent.DEFAULT_FORMAT.none;
			}

			if (typeof (this.format1.draggable) === 'undefined') {
				this.format1.draggable = VendorAssignDualListComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	direction() {
		return this.format1.direction === VendorAssignDualListComponent.LTR;
	}
