...
@Component({
  selector: 'app-smr-trn-invoiceedit',
  templateUrl: './smr-trn-invoiceedit.component.html',
  styleUrls: ['./smr-trn-invoiceedit.component.scss']
})


export class SmrTrnInvoiceeditComponent {
  ...
  GetEditInvoice(invoice_gid:any) {
    ...
    var api = 'SmrRptInvoiceReport/GetEditInvoice';
    this.service.getparams(api, param).subscribe((result: any) => {
      this.responsedata=result.InvoiceEdit_list;
      console.log(this.responsedata+'list of binded data'); 

      // const invoicedate = new Date(this.responsedata[0].invoice_date);
      ...
      // const duedate = new Date(this.responsedata[0].payment_date);
      ...
      this.combinedFormData.get("shipping_to")?.setValue(this.responsedata[0].shipping_to);
      this.combinedFormData.get("customer_mobile")?.setValue(this.responsedata[0].customer_contactnumber);
      ...
      const contactTelephonenumber = this.responsedata[0].customer_contactnumber;
      const gstNo = this.responsedata[0].gst_no;
      const customer_details = `${emailId}\n${contactTelephonenumber}\n${gstNo}`;
      ...
      this.combinedFormData.get("delivery_days")?.setValue(this.responsedata[0].delivery_days);
      this.combinedFormData.get("bill_email")?.setValue(this.responsedata[0].bill_email);
      ...
      this.combinedFormData.get("totalamount")?.setValue(this.responsedata[0].invoice_amount);
      this.combinedFormData.get("addon_charge")?.setValue(this.responsedata[0].additionalcharges_amount);
      this.combinedFormData.get("additional_discount")?.setValue(this.responsedata[0].discount_amount);
      this.combinedFormData.get("freight_charges")?.setValue(this.responsedata[0].freight_charges);
      this.combinedFormData.get("tax_name4")?.setValue(this.responsedata[0].tax_gid);
      this.combinedFormData.get("tax_amount4")?.setValue(this.responsedata[0].tax_amount);
  ...
  Geteditinvoiceproductsummary(invoice_gid:any){
    var url = 'SmrRptInvoiceReport/GetEditInvoiceProductSummary';
    let param={
      invoice_gid:invoice_gid
    }
    this.service.getparams(url,param).subscribe((result:any)=>{
      this.responsedata = result;
      this.editinvoiceproductsummary_list = result.editinvoiceproductsummarylist;
...
if (
  this.combinedFormData.value.customer_name == "" ||
   ...
   this.combinedFormData.value.customer_name == undefined &&
    ...
    var params = {
      ...
      customercontactnumber: this.combinedFormData.value.customer_mobile,
      customeremailaddress: this.combinedFormData.value.customer_email,
    ...
    var url = 'SmrRptInvoiceReport/UpdatedInvoice'
  ...
  finaltotal() {
    debugger
    {
      ...
      this.grandtotal = Number(totalAmount) + Number(total_amount) + Number(addoncharges) + Number(frieghtcharges) +
      Number(packing_charges) + Number(roundoff) - Number(discountamount);
  ...
  ondelete() {
    debugger
    var url = 'SmrRptInvoiceReport/GetDeletemainInvoiceProductSummary'
    this.NgxSpinnerService.show();
    let param = {
