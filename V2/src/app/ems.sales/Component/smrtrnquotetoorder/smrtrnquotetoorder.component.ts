...
export class SmrtrnquotetoorderComponent {
  ...
  ngOnInit(): void {
    ...
    this.GetQuotationSummary(this.quotation_gid);
  ...
  async GetQuotationSummary(quotation_gid: any) {

this.NgxSpinnerService.show();
    let param = { quotation_gid: quotation_gid }
    var summaryapi = 'SmrQuoteToOrder/GetRaiseSOSummary';
    this.service.getparams(summaryapi, param).subscribe((apiresponse: any) => {
    this.SO_list = apiresponse.SO_list;
    const address1 = this.SO_list[0].address1;
    const address2 = this.SO_list[0].address2;
    const zip_code = this.SO_list[0].zip_code;
    const city = this.SO_list[0].city;
    ...
    this.combinedFormData.get('tax_name4')?.setValue(this.SO_list[0].tax_name4);
    ...
    this.QuotationtoSOSummary(this.quotation_gid);
    this.NgxSpinnerService.hide();
  } 
  QuotationtoSOSummary(quotation_gid: any){
    
    
    let param = { quotation_gid : quotation_gid}
    this.NgxSpinnerService.show();
    var productsummaryapi = 'SmrQuoteToOrder/GetQuotationtoSOProductSummary';
  ...
  OnChangeBranch(branch_gid: any) {
    ...
    // let branch_gid = this.combinedFormData.value.branch_name;
  ...
  onSubmit() {
    ...
    var url = 'SmrQuoteToOrder/PostQuotationToOrder'
    this.NgxSpinnerService.show();
  ...
  finaltotal() {
    debugger
    {
      const total_amount = parseFloat(this.combinedFormData.value.tax_amount4.replace(/,/g, ''));
  ...
  SOproductsummary(quotation_gid:any) {
    
    this.NgxSpinnerService.show();
   let param ={ quotation_gid : quotation_gid}
    var api = 'SmrQuoteToOrder/GetTemporarySummary';

    this.service.getparams(api, param).subscribe((result: any) => {
      this.responsedata = result;
      this.salesorders_list = this.responsedata.temp_list;
