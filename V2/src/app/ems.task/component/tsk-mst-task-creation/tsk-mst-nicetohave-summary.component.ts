...
@Component({
  selector: 'app-tsk-mst-nicetohave-summary',
  templateUrl: './tsk-mst-nicetohave-summary.component.html',
  styles: [`.badge-blue {
    color: #2cacbd;
...
export class TskMstNicetohaveSummaryComponent {
  show_stopper: any;
  non_mandatory: any;
  parametervalue: any;
  ...
  ngOnInit() {
    ...
    this.SocketService.get(url).subscribe((result: any) => {
      if (result.taskpending_list != null) {
        $('#nice').DataTable().destroy();
        this.nice_summary = result.taskpending_list;
