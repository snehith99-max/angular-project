...
@Component({
  selector: 'app-tsk-mst-non-mandatorysummary',
  templateUrl: './tsk-mst-non-mandatorysummary.component.html',
  styles: [`.badge-blue {
    color: #2cacbd;
...
export class TskMstNonMandatorysummaryComponent {
  tasknonmandatory_list: any;
  mandatory: any;
  show_stopper: any;
  parametervalue: any;
  nice_to_count: any;
  ...
  ngOnInit() {
    ...
    this.SocketService.get(url).subscribe((result: any) => {
      if (result.taskpending_list != null) {
        $('#nonmandatory').DataTable().destroy();
        this.tasknonmandatory_list = result.taskpending_list;
        this.non_mandatory=result.non_mandatory
        this.NgxSpinnerService.hide();
        setTimeout(() => {
          $('#nonmandatory').DataTable();
        }, 1);
      }
      else {
        this.tasknonmandatory_list = result.taskpending_list;
        this.non_mandatory=result.non_mandatory
        setTimeout(() => {
          var table = $('#nonmandatory').DataTable();
        }, 1);
        this.NgxSpinnerService.hide();
        $('#nonmandatory').DataTable().destroy();
        this.NgxSpinnerService.hide();
