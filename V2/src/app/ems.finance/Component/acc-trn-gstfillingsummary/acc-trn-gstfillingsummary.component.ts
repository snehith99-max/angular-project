...
@Component({
  selector: 'app-acc-trn-gstfillingsummary',
  templateUrl: './acc-trn-gstfillingsummary.component.html',
  styleUrls: ['./acc-trn-gstfillingsummary.component.scss']
})
export class AccTrnGstfillingsummaryComponent {
  pick: Array<any> = [];
 ...
 constructor(private fb: FormBuilder, private route: ActivatedRoute, private router: Router, private service: SocketService, private ToastrService: ToastrService, private NgxSpinnerService: NgxSpinnerService) {
  ...
  ngOnInit(): void {
    ...
    var url = 'GstManagement/InFillingGstManagementSummary'
    ...
    var url = 'GstManagement/OutFillingGstManagementSummary'
    this.service.getparams(url, param).subscribe((result: any) => {
      this.responsedata = result;
