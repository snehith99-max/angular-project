...
@Component({
  selector: 'app-acc-mst-chartofaccountliability',
  templateUrl: './acc-mst-chartofaccountliability.component.html',
  styleUrls: ['./acc-mst-chartofaccountliability.component.scss']
})
export class AccMstChartofaccountliabilityComponent {
  ...
  Getchartofaccountliability_list: any;
  ...
  getsummary()
  {
    var url = 'ChartofAccount/ChartofAccountLiabilitySummary'
    this.service.get(url).subscribe((result: any) => {

      this.responsedata = result;
      this.Getchartofaccountliability_list = this.responsedata.Getchartofaccountliability_list;
      if (this.Getchartofaccountliability_list.length >= 0) {
       // console.log(this.Getchartofaccountliability_list)
        this.account_gid = this.Getchartofaccountliability_list[0].account_gid;
        this.account_code = this.Getchartofaccountliability_list[0].account_code;
        this.account_group = this.Getchartofaccountliability_list[0].account_name;
        ...
        this.service.getparams(url, param).subscribe((result: any) => {
          ...
          this.filteredCategories = this.Getchartofaccountliability_list;
        ...
        // console.log(this.Getchartofaccountliability_list[0].account_gid)
      }
      else {
        let param = {
          account_gid: ''
      ...
      setTimeout(() => {
        $('#Getchartofaccountliability_list').DataTable(
          {
            // code by snehith for customized pagination
            "pageLength": 25, // Number of rows to display per page
            "lengthMenu": [25, 50, 100, 200], // Dropdown to change page length
  ...
  applyFilter(): void {
    if (!this.searchText) {
      this.filteredCategories = this.Getchartofaccountliability_list; // Show all categories if search text is empty
    } else {
      this.filteredCategories = this.Getchartofaccountliability_list.filter((category: { account_name: string; }) =>
  ...
  childview(account_gid: any, account_code: any, account_name: any) {
    ...
    this.router.navigate(['/finance/AccMstChartofAccountLiabilityView', lsaccount_gid, lsaccount_code, lsaccount_name]);
  }
  get accountcode() {
  ...
  initForm(): void {
    this.ledgerForm = this.fb.group({
      ...
      accountType: ['BS'], // Default to 'BS' (Profit & Loss)
      displayType: ['liability'], // Default to 'liability' for 'BS'
    ...
    if (accountTypeControl) { // Check if accountTypeControl is not null
      accountTypeControl.valueChanges.subscribe((value: string) => {
        ...
        if (displayTypeControl) { // Check if displayTypeControl is not null
          if (value === 'PL') {
            displayTypeControl.setValue('expense'); // Set default to 'expense' for 'BS'
          } else if (value === 'BS') {
            displayTypeControl.setValue('liability'); // Set default to 'asset' for 'BS'
          }
        }
      });
    }
