...
@Component({
  selector: 'app-acc-trn-editsundryexpenses',
  templateUrl: './acc-trn-editsundryexpenses.component.html',
  styleUrls: ['./acc-trn-editsundryexpenses.component.scss']
})
export class AccTrnEditsundryexpensesComponent {
  ...
  expense_gid : any;
  expenseview : any;
  editexpense_list : any[]=[];
  editsundryproexpense_list : any[]=[];
  editproexpense_list : any[]=[];
  address1 : any;
  address2: any;
  city: any;
  state : any;
  ...
  this.EditExpenseSummary(this.expense_gid);
...
this.productSearch();
    ...
    this.GetViewexpensesProductSummary(this.expense_gid);
    this.onchangeaccountgroup(this.subgroup_gid);
  }

  onchangeaccountgroup(subgroup_gid: any) {
    if(subgroup_gid == null || subgroup_gid == "")
    {
      var url = "AccTrnSundryExpenses/GetAllLedgerList";
    ...
    let param = {
      account_gid: subgroup_gid
    }
    this.service.getparams(url, param).subscribe((result: any) => {
      this.accountgroup_lists = result.GetAccGroup;
      this.productform.get("Account_name")?.setValue(result.accountgroup_lists[0].account_name);
    });
  }
  }
  ...
  EditExpenseSummary(expense_gid: any){
    let param ={
      expense_gid :expense_gid
    }
    var url = 'AccTrnSundryExpenses/GetSundryExpenseEdit';
    this.service.getparams(url,param).subscribe((result:any)=>{
      this.editexpense_list = result.expense_list;

      this.address1 = this.editexpense_list[0].address1;  
      this.address2 = this.editexpense_list[0].address2;  
      this.city = this.editexpense_list[0].city;
      this.state = this.editexpense_list[0].state;
      this.pincode = this.editexpense_list[0].postal_code;
  
      this.contactperson_name = this.editexpense_list[0].contactperson_name;
      this.email_id = this.editexpense_list[0].email_id;
      this.contact_telephonenumber = this.editexpense_list[0].contact_telephonenumber;
  
      const address = this.address1 + '\n' + this.address2 + '\n' +this.city +'\n' + this.state +'\n'+this.pincode;
      this.vendor_address1 = address
  
      const address2 = this.contactperson_name + '\n' + this.email_id + '\n' +this.contact_telephonenumber ;
      this.vendor_address2 = address2

      this.sundryexpenseForm.get("vendor_companyname")?.setValue(this.editexpense_list[0].vendor_gid);
      this.sundryexpenseForm.get("invoice_ref_no")?.setValue(this.editexpense_list[0].expense_reference);
      this.sundryexpenseForm.get("invoice_date")?.setValue(this.editexpense_list[0].expense_date);
      this.sundryexpenseForm.get("vendor_details")?.setValue(this.vendor_address2);
      this.sundryexpenseForm.get("address1")?.setValue(this.vendor_address1);
      this.sundryexpenseForm.get("shipping_address")?.setValue(this.editexpense_list[0].ship_to);
      this.sundryexpenseForm.get("exchange_rate")?.setValue(this.editexpense_list[0].exchange_rate);
      this.sundryexpenseForm.get("currency_code")?.setValue(this.editexpense_list[0].currency_code);
      this.sundryexpenseForm.get("payment_terms")?.setValue(this.editexpense_list[0].payment_terms);
      this.sundryexpenseForm.get("due_date")?.setValue(this.editexpense_list[0].due_date);
      this.sundryexpenseForm.get("invoice_remarks")?.setValue(this.editexpense_list[0].invoice_remarks);
      this.sundryexpenseForm.get("payment_terms")?.setValue(this.editexpense_list[0].payment_term);
      this.sundryexpenseForm.get("branch_name")?.setValue(this.editexpense_list[0].branch_gid);
    })
  }
  ...
  get invoice_date() {
    return this.sundryexpenseForm.get('invoice_date')!;
  }
  get invoice_ref_no() {
    return this.sundryexpenseForm.get('invoice_ref_no')!;
  }
get purchase_type(){
...
this.grandtotal = this.total_amount + (+this.tax_amount4) + (+this.addoncharge) + (+this.freightcharges) +
  ...
  ondelete() {
    var url = 'AccTrnSundryExpenses/EditTempProductDelete'
  ...
  productAddSubmit() {
    if (this.sundryexpenseForm.value.vendor_companyname == "" || this.sundryexpenseForm.value.vendor_companyname == null || this.sundryexpenseForm.value.vendor_companyname == undefined) {
      window.scrollTo({
        top: 0,
      });
    ...
    var postapi = 'AccTrnSundryExpenses/PostProductUpdatesundryexpenses';
    this.NgxSpinnerService.show();
    let param = {
      expense_gid : this.expense_gid,
      product_name: this.productform.value.product_name,
  ...
  onSubmit() {
    ...
    var params = {
      ...
      expense_ref_no: this.sundryexpenseForm.value.invoice_ref_no,
      expense_date: this.sundryexpenseForm.value.invoice_date,
      ...
      ledgersEdit_list : this.editsundryproexpense_list
    };
    var api = 'AccTrnSundryExpenses/UpdateSundryExpenses';
    this.NgxSpinnerService.show()
...
//   }
  ...
  GetViewexpensesProductSummary(expense_gid : any) {
    var url='AccTrnSundryExpenses/GetSundryExpenseEditProducts'
    let param = {
      expense_gid : expense_gid 
    }
    this.service.getparams(url,param).subscribe((result:any)=>{
    this.responsedata=result;
    this.editsundryproexpense_list = result.sundryledgerview_list; 
    console.log(this.editsundryproexpense_list,'list')
