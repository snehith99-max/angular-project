...
@Component({
  selector: 'app-acc-mst-chartofaccount',
  templateUrl: './acc-mst-chartofaccount.component.html',
  styleUrls: ['./acc-mst-chartofaccount.component.scss'],
  styles: [`
table thead th, 
.table tbody td { 
 position: relative; 
...
export class AccMstChartofaccountComponent {
  sampleList: any;
  account_gid: any;
  ...
  //     this.mainListTwo = result.incomefolder.map((item: any) => ({ ...item, visible: false }));
  //     this.assetfolder = result.assetfolder.map((item: any) => ({ ...item, visible: false }));
  //     this.liabilityfolder = result.liabilityfolder.map((item: any) => this.transformSubfolderData(item));
  //     this.subList = result.subfolders.map((item: any) => ({ ...item, visible: false }));
  //     this.NgxSpinnerService.hide();
  ...
  // // Function to transform subfolder data
  // transformSubfolderData(folder: any) {
  //   if (folder.subfolders && folder.subfolders.length > 0) {
  //     folder.subfolders.forEach((subfolder: any) => {
  //       subfolder.account_name = "Account Sub Group Ledger_001"; // Desired ledger name
  //       subfolder.accountgroup_name = folder.account_name; // Parent account name
  //     });
  //   }
...
// toggleswitch(event: Event): void {
  //   var api = 'ChartofAccount/UpdateAccountSubGroup'
  //   var params = {
  //     ledger_flag : event,
