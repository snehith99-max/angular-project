...
@Component({
  selector: 'app-acc-mst-chartofaccountasset',
  templateUrl: './acc-mst-chartofaccountasset.component.html',
  styleUrls: ['./acc-mst-chartofaccountasset.component.scss']
})
export class AccMstChartofaccountassetComponent {
  ...
  Getchartofaccountasset_list: any;
  ...
  getsummary()
  {
    var url = 'ChartofAccount/ChartofAccountAssetSummary'
    this.service.get(url).subscribe((result: any) => {

      this.responsedata = result;
      this.Getchartofaccountasset_list = this.responsedata.Getchartofaccountasset_list;
      if (this.Getchartofaccountasset_list.length >= 0) {
      //  console.log(this.Getchartofaccountasset_list)
        this.account_gid = this.Getchartofaccountasset_list[0].account_gid;
        this.account_code = this.Getchartofaccountasset_list[0].account_code;
        this.account_group = this.Getchartofaccountasset_list[0].account_name;
        ...
        this.service.getparams(url, param).subscribe((result: any) => {
          ...
          this.filteredCategories = this.Getchartofaccountasset_list;
        ...
        // console.log(this.Getchartofaccountasset_list[0].account_gid)
      }
      else {
        let param = {
          account_gid: ''
      ...
      setTimeout(() => {
        $('#Getchartofaccountasset_list').DataTable(
          {
            // code by snehith for customized pagination
            "pageLength": 25, // Number of rows to display per page
            "lengthMenu": [25, 50, 100, 200], // Dropdown to change page length
  ...
  applyFilter(): void {
    if (!this.searchText) {
      this.filteredCategories = this.Getchartofaccountasset_list; // Show all categories if search text is empty
    } else {
      this.filteredCategories = this.Getchartofaccountasset_list.filter((category: { account_name: string; }) =>
  ...
  childview(account_gid: any, account_code: any, account_name: any) {
    ...
    this.router.navigate(['/finance/AccMstChartofAccountAssetView', lsaccount_gid, lsaccount_code, lsaccount_name]);
  }
  get accountcode() {
  ...
  initForm(): void {
    this.ledgerForm = this.fb.group({
      ...
      accountType: ['BS'], // Default to 'BS' (BS)
      displayType: ['asset'], // Default to 'asset' for 'BS'
