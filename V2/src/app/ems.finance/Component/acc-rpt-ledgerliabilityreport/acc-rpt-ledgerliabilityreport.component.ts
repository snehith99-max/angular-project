...
@Component({
  selector: 'app-acc-rpt-ledgerliabilityreport',
  templateUrl: './acc-rpt-ledgerliabilityreport.component.html',
  styleUrls: ['./acc-rpt-ledgerliabilityreport.component.scss']
})
export class AccRptLedgerliabilityreportComponent {

  account_gid: any;
  CreditDetailedForm!: FormGroup;
  responsedata: any;
  LiabilityLedgerReportView_List: any[] = [];
  ...
  GetVendorName(account_gid: any) {
    ...
    var url = 'AccTrnBankbooksummary/GetLiabilityLedgerReportDetails'
    let param = {
      account_gid: account_gid,
    }
    this.service.getparams(url, param).subscribe((result: any) => {
      this.responsedata = result;
      this.Creditorvendor = result.GetLiabilityLedgerReportDetails_List;
  ...
  GetAccTrnCreditorReportView(account_gid: any, from_date: any, to_date: any) {
    this.NgxSpinnerService.show();
    var url = 'AccTrnBankbooksummary/GetLiabilityLedgerReportDetailsList'
    let param = {
      account_gid: account_gid,
      from_date: from_date,
      to_date: to_date
    ...
    this.service.getparams(url, param).subscribe((result: any) => {
      this.responsedata = result;
      this.LiabilityLedgerReportView_List = result.GetLiabilityLedgerReportView_List;
      console.log(this.LiabilityLedgerReportView_List,'list')
      if (this.LiabilityLedgerReportView_List != null) {
        this.closingamount = this.LiabilityLedgerReportView_List.reduce((total: any, data: any) => total + parseFloat(data.closingbalance.replace(',', '')), 0);
        this.totalCredit = this.LiabilityLedgerReportView_List.reduce((total: any, data: any) => total + parseFloat(data.credit_amount.replace(',', '')), 0);
        this.totalDebit = this.LiabilityLedgerReportView_List.reduce((total: any, data: any) => total + parseFloat(data.debit_amount.replace(',', '')), 0);
      ...
      setTimeout(() => {
        $('#LiabilityLedgerReportView_List').DataTable();
      }, 1);
      this.NgxSpinnerService.hide();
    });
  }
  ...
  exportExcel(): void {
    const CreditorreportViewList = this.LiabilityLedgerReportView_List.map(item => ({
  ...
  onSearchClick() {
    ...
    this.service.getparams(url, params).subscribe((result: any) => {
      $('#LiabilityLedgerReportView_List').DataTable().destroy();
      this.responsedata = result;
      this.LiabilityLedgerReportView_List = result.GetLiabilityLedgerReportView_List;
      if (this.LiabilityLedgerReportView_List != null) {
        this.closingamount = this.LiabilityLedgerReportView_List.reduce((total: any, data: any) => total + parseFloat(data.closingbalance.replace(',', '')), 0);
