...
@Component({
  selector: 'app-pbl-trn-invoiceaddselectgrndetails',
  templateUrl: './pbl-trn-invoiceaddselectgrndetails.component.html',
  styleUrls: ['./pbl-trn-invoiceaddselectgrndetails.component.scss']
})
export class PblTrnInvoiceaddselectgrndetailsComponent {
...
purchasetype_list : any[]=[];
  constructor(private formBuilder: FormBuilder,
    ...
    invoice_remarks : new FormControl(''),
      ...
      grandtotal1: new FormControl(''),
      roundoff1: new FormControl(''),
      tax_amount41: new FormControl(''),
      tax_name41: new FormControl(''),
      freight_charges1: new FormControl(''),
      additional_discount1: new FormControl(''),
      addon_charge1: new FormControl(''),
      totalamount1: new FormControl(''),
  ...
  ngOnInit(): void {
    ...
    const vendor_gid1 = this.route.snapshot.paramMap.get('vendor_gid2');
    const purchaseorder_gid1 = this.route.snapshot.paramMap.get('purchaseorder_gid1');
    const grn_gid = this.route.snapshot.paramMap.get('grn_gid3');
    ...
    this.GetViewPurchaseOrderSummary(purchaseorder_gid_1,grn_gid1);
    this.GetViewPurchaseOrderProduct(purchaseorder_gid_1);
...
this.grandtotal = this.total_amount + (+this.tax_amount4) + (+this.addoncharge) + (+this.freightcharges) +
  ...
  GetViewPurchaseOrderSummary(purchaseorder_gid_1: any,grn_gid1:any) {
    debugger
    var url='PblInvoiceGrnDetails/GetPurchaseInvoicesummary'  
    let param = {
    ...
    this.InvoiceForm.get("total_amount")?.setValue(this.purchaseorder_list[0].total_amount);
    ...
    this.tax_amount4 = this.purchaseorder_list[0].overalltax;
    this.mdlTaxName4 = this.purchaseorder_list[0].tax_gid;
    ...
    this.addoncharge = this.purchaseorder_list[0].addon_amount;
   

    this.InvoiceForm.get("totalamount1")?.setValue(this.purchaseorder_list[0].poproducttotal);
    this.InvoiceForm.get("addon_charge1")?.setValue(this.purchaseorder_list[0].poaddonamount);
    this.InvoiceForm.get("additional_discount1")?.setValue(this.purchaseorder_list[0].podiscountamount);
    this.InvoiceForm.get("freight_charges1")?.setValue(this.purchaseorder_list[0].pofreightcharges);
    this.InvoiceForm.get("tax_name41")?.setValue(this.purchaseorder_list[0].potaxgid);
    this.InvoiceForm.get("tax_amount41")?.setValue(this.purchaseorder_list[0].potaxamount);
    this.InvoiceForm.get("roundoff1")?.setValue(this.purchaseorder_list[0].poroundoff);
    this.InvoiceForm.get("grandtotal1")?.setValue(this.purchaseorder_list[0].pototalamount);
   
    });
  }
  GetViewPurchaseOrderProduct(purchaseorder_gid_1: any) {
     
    var url='PblInvoiceGrnDetails/GetPurchaseInvoiceproduct'  
    let param = {
      purchaseorder_gid : purchaseorder_gid_1 
    }
    this.service.getparams(url,param).subscribe((result:any)=>{
      this.purchaseorderproduct_list = result.GetinvioceProduct;
      // this.InvoiceForm.get("product_total")?.setValue(result.product_total)
       this.InvoiceForm.get("netamount")?.setValue(result.grand_total)
  ...
  onSubmit() {
    ...
    var params = {
      ...
      currency_code : this.InvoiceForm.value.currency_code
    }
