...
@Component({
  selector: 'app-pbl-trn-directinvoice-edit',
  templateUrl: './pbl-trn-directinvoice-edit.component.html',
  styleUrls: ['./pbl-trn-directinvoice-edit.component.scss']
})
export class PblTrnDirectinvoiceEditComponent {
...
this.grandtotal = this.total_amount + (+this.tax_amount4) + (+this.addoncharge) + (+this.freightcharges) +
  ...
  onSubmit() {
    ...
    var api = 'PmrTrnDirectInvoice/DaPblProductUpdate';
    this.NgxSpinnerService.show()
  ...
  GetEditProductSummary(invoice_gid: any) {
    debugger
    var url = 'PmrTrnDirectInvoice/GetPmrTrnInvoiceedit'
       let param = {invoice_gid : invoice_gid}
    this.service.getparams(url, param).subscribe((result: any) => {
      this.responsedata=result;
      this.invoice_listsedit = result.invoice_listsedit;      // this.product = result;
