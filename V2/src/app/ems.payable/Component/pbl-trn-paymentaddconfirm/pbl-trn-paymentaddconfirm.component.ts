...
@Component({
    selector: 'app-pbl-trn-paymentaddconfirm',
    templateUrl: './pbl-trn-paymentaddconfirm.component.html',
    styleUrls: ['./pbl-trn-paymentaddconfirm.component.scss']
  })
  export class PblTrnPaymentaddconfirmComponent {
...
debugger
  ...
  this.invoice_gid_key = invoice_gid;
  this.vendor_gid_key = vendor_gid;
  const invoice_gid1 = AES.decrypt(this.invoice_gid_key, secretKey).toString(enc.Utf8);
  const vendor_gid1 = AES.decrypt(this.vendor_gid_key, secretKey).toString(enc.Utf8);;
   this.GetsingleSummary(invoice_gid1,vendor_gid1);
    debugger
  ...
  GetsingleSummary(invoice_gid:any,vendor_gid: any) {
    debugger
    var param = {
  ...
  debugger
    var url = 'PblTrnPaymentRpt/GetSingleaddPaymentSummary'
    this.NgxSpinnerService.show();
    this.service.getparams(url, param1).subscribe((result: any) => {
      this.responsedata = result;
      this.singlepayment_list = this.responsedata.singleinvoicelist;
      ...
      // this.reactiveForm.get("remark")?.setValue(result.GetBranch[0].invoice_remarks);
      for (let i = 0; i < this.singlepayment_list.length; i++) {
        this.reactiveForm.addControl(`advance${i}`, new FormControl(this.singlepayment_list[i].advance_amount));
  ...
  onsubmit() {
    ...
    this.NgxSpinnerService.show();
    var api = 'PblTrnPaymentRpt/Postsinglepayment';
