...
@Component({
  selector: 'app-ims-mst-product-summary',
  templateUrl: './ims-mst-product-summary.component.html',
  styleUrls: ['./ims-mst-product-summary.component.scss'],
  styles: [`
  table thead th, 
  .table tbody td { 
   position: relative; 
...
export class ImsMstProductSummaryComponent {
  employee!: IEmployee;
  log_list : any[]=[];
  ...
  onChange2(event:any) {
    ...
    onedit(params:any){
      ...
      this.router.navigate(['/ims/ImsMstProductEdit',encryptedParam]) 
    }
    onview(params:any){
      const secretKey = 'storyboarderp';
      const param = (params);
      const encryptedParam = AES.encrypt(param,secretKey).toString();
      this.router.navigate(['/ims/ImsMstProductView',encryptedParam])
    }
  
  onadd()
  {
        this.router.navigate(['/ims/ImsMstProductAdd'])
