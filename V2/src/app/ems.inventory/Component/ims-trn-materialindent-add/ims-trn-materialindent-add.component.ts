...
@Component({
  selector: 'app-ims-trn-materialindent-add',
  templateUrl: './ims-trn-materialindent-add.component.html',
  styleUrls: ['./ims-trn-materialindent-add.component.scss']
})
export class ImsTrnMaterialindentAddComponent {
  ...
  onSubmit() {
    ...
    this.service.postparams(api, params).subscribe((result: any) => {
      if (result.status == false) {
        ...
        this.ToastrService.success(result.message)
        this.router.navigate(['/ims/ImsTrnMaterialindent']);
      }
      this.NgxSpinnerService.hide()
    });
