...
@Component({
  selector: 'app-pmr-trn-directpo-add-latest',
  templateUrl: './pmr-trn-directpo-add-latest.component.html',
  styleUrls: ['./pmr-trn-directpo-add-latest.component.scss']
})
export class PmrTrnDirectpoAddLatestComponent {
  ...
  constructor(private fb: FormBuilder, private route: ActivatedRoute, 
    private router: Router, private service: SocketService,
  ...
  OnChangerequestor() {
    // debugger
    //    let user_gid = this.POAddForm.get("employee_name")?.value;
    //    let param ={
    //     user_gid :user_gid
    //    }
    //    var url = 'PmrTrnPurchaseOrder/GetOnChangeVendor';
  ...
  onSubmit() {
    ...
    if (this.file != null && this.file != undefined) {
      ...
      const api = 'PmrTrnPurchaseOrder/PostPurchaseOrderfileupload';
      this.NgxSpinnerService.show();
  ...
  ondraft() {
    ...
    if (this.file != null && this.file != undefined) {
      ...
      const api = 'PmrTrnPurchaseOrder/PostPurchaseOrderfileuploaddraft';
      this.NgxSpinnerService.show();
      this.service.post(api, formData).subscribe((result: any) => {
        if (result.status == false) {
          this.NgxSpinnerService.hide();
      ...
      var api = 'PmrTrnPurchaseOrder/ProductSubmitdraft';
      this.NgxSpinnerService.show()
      this.service.postparams(api, params).subscribe((result: any) => {
        if (result.status == false) {
          this.NgxSpinnerService.hide()
  ...
  GetSmrTrnSalesorderDraft() {
    var url = 'PmrTrnPurchaseOrder/GetPoDraftsSummary'
    this.NgxSpinnerService.show()
    this.service.get(url).subscribe((result: any) => {
      $('#PoDraftsSummary').DataTable().destroy();
      this.responsedata = result;
      this.PoDraftsSummary = this.responsedata.GetDraft;
      this.NgxSpinnerService.hide()
      
      setTimeout(() => {
        $('#PoDraftsSummary').DataTable();
      }, 1);
  ...
  Drafts(params:any){
    debugger
    var url = 'PmrTrnPurchaseOrder/GetDraftPurchaseOrderSummary'
    let param = {
      purchaseorderdraft_gid : params 
    }
    this.service.getparams(url, param).subscribe((result: any) => {
