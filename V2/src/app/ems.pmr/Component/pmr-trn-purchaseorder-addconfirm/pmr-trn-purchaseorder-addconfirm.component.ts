...
@Component({
  selector: 'app-pmr-trn-purchaseorder-addconfirm',
  templateUrl: './pmr-trn-purchaseorder-addconfirm.component.html',
  styleUrls: ['./pmr-trn-purchaseorder-addconfirm.component.scss']
})
export class PmrTrnPurchaseorderAddconfirmComponent {
...
getCurrentDate(): string {
  ...
  flatpickr('.date-picker', options);
   ...
   this.POproductsummary(purchaserequisition_gid1);
...
POproductsummary(purchaserequisition_gid1 : any) {
  let param = {
    purchaserequisition_gid : purchaserequisition_gid1
  }
  const url = 'PmrTrnPurchaseorderAddselect/GetPurchaseOrderSelectSummary';
  this.service.getparams(url,param).subscribe((result: any) => {
    this.responsedata = result;
...
productSearch() {
  ...
  let purchaserequisition_gid = this.purchaserequisition_gidenc;
  let param = {
    purchaserequisition_gid: purchaserequisition_gid,
    vendor_gid: vendorregister_gid
  }
  const url = 'PmrTrnPurchaseorderAddselect/GetPurchaseOrderSelectSummarytax';
  this.service.getparams(url, param).subscribe((result: any) => {
    this.responsedata = result;
    this.GetPurchaseOrder1 = this.responsedata.GetPurchaseOrder1;
...
calculateProductTotal1(product: any) {
  debugger
      ...
      formData.append('renewal_mode', this.POAddForm.value.renewal_mode);
      formData.append('taxsegment_gid', this.Cmntaxsegment_gid);

    
      this.NgxSpinnerService.show();
      const url = 'PmrTrnPurchaseorderAddselect/PostPurchaseOrderfileupload';
      this.service.post(url, formData).subscribe((result: any) => {
        ...
        if (result.status === false) {
          ...
          this.NgxSpinnerService.show();    
           const url = 'PmrTrnPurchaseorderAddselect/PostOverallSubmit';    
