...
import { VendorBasicList } from './vendorbasic-list1'
...
export class ICampaignUnAssign {
	...
	vendorcampaignunassignemp: string[] = [];
}
...
@Component({
  selector: 'vendor-assign-dual-list1',
  templateUrl: './vendor-assign-dual-list1.component.html',
  styleUrls: ['./vendor-assign-dual-list1.component.scss']
})

export class VendorAssignDualList1Component implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'UnAssigned Vendors', remove: 'Assigned Vendors', all: 'All Select', none: 'All Unselect', direction: VendorAssignDualList1Component.LTR, draggable: true };

	@Input() id = `dual-list-${nextId++}`;
	@Input() key2 = '_id';
	@Input() key3 = '_key3';
	@Input() taxsegment_gid = '_key3';
	@Input() display1 = '_name';
	@Input() height = '100px';
	@Input() filter1 = false;
	@Input() format1 = VendorAssignDualList1Component.DEFAULT_FORMAT;
	@Input() sort1 = false;
	@Input()
	...
	available: VendorBasicList;
	confirmed: VendorBasicList;
	sourceDiffer: any;
	destinationDiffer: any;
	private sorter = (a: any, b: any) => { return (a._name < b._name) ? -1 : ((a._name > b._name) ? 1 : 0); };


	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: PmrMstTaxsegmentComponent, private NgxSpinnerService: NgxSpinnerService) {
		this.available = new VendorBasicList(VendorAssignDualList1Component.AVAILABLE_LIST_NAME);
		this.confirmed = new VendorBasicList(VendorAssignDualList1Component.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: { [key2: string]: SimpleChange }) {
		...
		if (changeRecord['format1']) {
			...
			if (typeof (this.format1.direction) === 'undefined') {
				this.format1.direction = VendorAssignDualList1Component.LTR;
			}

			if (typeof (this.format1.add) === 'undefined') {
				this.format1.add = VendorAssignDualList1Component.DEFAULT_FORMAT.add;
			}

			if (typeof (this.format1.remove) === 'undefined') {
				this.format1.remove = VendorAssignDualList1Component.DEFAULT_FORMAT.remove;
			}

			if (typeof (this.format1.all) === 'undefined') {
				this.format1.all = VendorAssignDualList1Component.DEFAULT_FORMAT.all;
			}

			if (typeof (this.format1.none) === 'undefined') {
				this.format1.none = VendorAssignDualList1Component.DEFAULT_FORMAT.none;
			}

			if (typeof (this.format1.draggable) === 'undefined') {
				this.format1.draggable = VendorAssignDualList1Component.DEFAULT_FORMAT.draggable;
			}
		}
	...
	direction() {
		return this.format1.direction === VendorAssignDualList1Component.LTR;
	}

	dragEnd(list: VendorBasicList) {
		if (list) {
	...
	drag(event: DragEvent, item: any, list: VendorBasicList) {
	...
	allowDrop(event: DragEvent, list: VendorBasicList) {
	...
	drop(event: DragEvent, list: VendorBasicList) {
	...
	private makeUnavailable(source: VendorBasicList, item: any) {
	...
	moveItem(source: VendorBasicList, target: VendorBasicList, item: any = null, trueup = true) {
		let i = 0;
	...
	onUpdate() {
		if (this.confirmed.sift.length > 0) {
			this.taxsegment_gid = this.confirmed.sift[0]._key3
		}
		else {
			this.taxsegment_gid = this.available.sift[0]._key3
		}
		this.selectAll(this.available);
		this.selectAll(this.confirmed);
		this.addbtn(this.available, this.confirmed, null, false, this.taxsegment_gid);
	}
	...
	addbtn(source: VendorBasicList, target: VendorBasicList, item: any = null, trueup = true, taxsegment_gid: string) {
		...
		const campaignUnassignData: ICampaignUnAssign = { taxsegment_gid:taxsegment_gid, vendorcampaignunassignemp:  [] };
		if (target.list.length === 0) {
			campaignUnassignData.vendorcampaignunassignemp  = target.sift.map((item: any) => item._id); // Pass all values from the target list
		} else {
			campaignUnassignData.vendorcampaignunassignemp  = target.pick; // Pass selected values only
		}
		this.NgxSpinnerService.show();
		const url = 'SmrMstTaxSegment/PostVendorUnassignedlist';
	...
	shiftClick(event: MouseEvent, index: number, source: VendorBasicList, item: any) {
	...
	selectAll(source: VendorBasicList) {
		source.pick.length = 0;
		source.pick = source.sift.slice(0);
	}

	selectNone(source: VendorBasicList) {
		source.pick.length = 0;
	}

	isAllSelected(source: VendorBasicList) {
	...
	isAnySelected(source: VendorBasicList) {
	...
	private unpick(source: VendorBasicList) {
	...
	clearFilter(source: VendorBasicList) {
		if (source) {
			source.picker = '';
			this.onFilter(source);
		}
	...
	onFilter(source: VendorBasicList) {
