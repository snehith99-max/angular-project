...
@Component({
  selector: 'app-pmr-trn-vendor360',
  templateUrl: './pmr-trn-vendor360.component.html',
  styleUrls: ['./pmr-trn-vendor360.component.scss']
})
export class PmrTrnVendor360Component {
  ...
  ngOnInit() : void{
    debugger
    this.vendor_gid = this.route.snapshot.paramMap.get('vendor_gid');
  ...
  GetCount(deencryptedParam : any) {
    var url = 'PmrVendor360/GetTilesCount';
    let param = {
  ...
  Getcustomerdetails(deencryptedParam : any) {
    debugger
    var url = 'PmrVendor360/GetVendorDetails'
    let param = {
      vendor_gid: this.vendor_gid
    }
    this.service.getparams(url, param).subscribe((result: any) => {
  ...
  GetPurchaseStatus(deencryptedParam : any) {
    var url = 'PmrVendor360/GetPurchasetatus';
    let param ={
      vendor_gid : this.vendor_gid
    }
    this.service.getparams(url,param).subscribe((result: any) => {
  ...
  getpaymentchart(deencryptedParam : any){
    var url = 'PmrVendor360/GetPaymentCount';
    let param ={
      vendor_gid : this.vendor_gid
    }
    this.service.getparams(url,param).subscribe((result: any) => {
  ...
  onback(){
    if(this.lspage == 'Finance'){
      this.router.navigate(['/finance/PmrTrnPurchaseLegderFin']);
     }
     else{
       this.router.navigate(['/pmr/PmrTrnPurchaseLedger']);
     }
  }
