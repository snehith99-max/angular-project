...
@Component({
  selector: 'app-pmr-trn-purchaseorder-edit',
  templateUrl: './pmr-trn-purchaseorder-edit.component.html',
})
export class PmrTrnPurchaseorderEditComponent {
  ...
  allchargeslist: any[] = [];
  GetEditPurchaseOrderSummary: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.GeteditPurchaseOrderSummary(deencryptedParam);
    ...
    this.service.get(api).subscribe((result: any) => {
      ...
      this.POAddForm.get("user_gid")?.setValue(this.user_list[0].user_gid);
  ...
  constructor(private fb: FormBuilder, private route: ActivatedRoute, private router: Router, private service: SocketService, private ToastrService: ToastrService, public NgxSpinnerService: NgxSpinnerService) {
    ...
    this.POAddForm = new FormGroup({
      ...
      renewal_date: new FormControl(Validators.required),
  ...
  OnChangeVendor() {
    ...
    this.service.getparams(url, param).subscribe((result: any) => {
      ...
      let address1 = this.GetVendord[0].address1;
      const address2 = this.GetVendord[0].address2;

      if (address1 === null || address1 === undefined || address1.trim() === '') {
        address1 = address2;
      }
      this.POAddForm.get("address1")?.setValue(address1);
      this.POAddForm.get("address2")?.setValue(address2);
      // this.POAddForm.get("shipping_address")?.setValue(address2);
  ...
  finaltotal() {
    ...
    const addoncharge = isNaN(this.POAddForm.value.addoncharge) ? 0 : +this.addoncharge;
    const freightcharges = isNaN(this.POAddForm.value.freightcharges) ? 0 : +this.freightcharges;
    const packing_charges = isNaN(this.POAddForm.value.packing_charges) ? 0 : +this.packing_charges;
    const insurance_charges = isNaN(this.insurance_charges) ? 0 : +this.insurance_charges;
    const roundoff = isNaN(this.POAddForm.value.roundoff) ? 0 : +this.roundoff;
    const additional_discount = isNaN(this.POAddForm.value.additional_discount) ? 0 : +this.additional_discount;
    const tax_amount4 = isNaN(this.POAddForm.value.tax_amount4) ? 0 : +this.tax_amount4;
    const totalamount = isNaN(this.POAddForm.value.totalamount) ? 0 : +this.totalamount;
  ...
  onSubmit() {
    ...
    if (this.file != null && this.file != undefined) {
      ...
      formData.append('vendor_details', this.POAddForm.value.vendor_details);
      ...
      formData.append('dispatch_mode', this.POAddForm.value.dispatch_mode);
      ...
      formData.append('tax_name1', this.POAddForm.value.tax_name1);
      formData.append('tax_name2', this.POAddForm.value.tax_name2);
      formData.append('tax_name3', this.POAddForm.value.tax_name3);
      formData.append('taxamount1', this.POAddForm.value.taxamount1);
      formData.append('taxamount2', this.POAddForm.value.taxamount2);
      formData.append('taxamount3', this.POAddForm.value.taxamount3);
      ...
      formData.append("renewal_flag",this.POAddForm.value.renewal_mode);
      formData.append("renewal_date",this.POAddForm.value.renewal_date);
      formData.append("frequency_terms",this.POAddForm.value.frequency_terms);
      formData.append("purchaseorder_gid",this.POAddForm.value.purchaseorder_gid);

      const api = 'PmrTrnPurchaseOrder/UpdatePurchaseOrderfileupload';
      this.NgxSpinnerService.show();
    ...
    var params = {
      ...
      renewal_mode: this.POAddForm.value.renewal_mode,
      renewal_date: this.POAddForm.value.renewal_date,
      frequency_terms: this.POAddForm.value.frequency_terms,

    };
    var api = 'PmrTrnPurchaseOrder/PoSubmit';
    this.NgxSpinnerService.show()
  ...
  productAddSubmit() {
    ...
    var postapi = 'PmrTrnPurchaseOrder/PostProductAdd';
    this.NgxSpinnerService.show();
  debugger
    let param = {
      product_name: this.productform.value.product_name,
  ...
  GeteditPurchaseOrderSummary(purchaseorder_gid: any) {
    debugger
    var url = 'PmrTrnPurchaseOrder/GetEditPurchaseOrderSummary'
    let param = {
    ...
    this.service.getparams(url, param).subscribe((result: any) => {
      ...
      this.responsedata = result;
      this.GetEditPurchaseOrderSummary = result.GetEditPurchaseOrderSummary;


      this.POAddForm.get("address_gid")?.setValue(this.GetEditPurchaseOrderSummary[0].address_gid);
      this.POAddForm.get("branch_name")?.setValue(this.GetEditPurchaseOrderSummary[0].branch_gid);
      this.POAddForm.get("expected_date")?.setValue(this.GetEditPurchaseOrderSummary[0].expected_date);
     
      this.POAddForm.get("po_no")?.setValue(this.GetEditPurchaseOrderSummary[0].poref_no);
      this.POAddForm.get("purchaseorder_gid")?.setValue(this.GetEditPurchaseOrderSummary[0].purchaseorder_gid);
      this.POAddForm.get("vendor_companyname")?.setValue(this.GetEditPurchaseOrderSummary[0].vendor_gid);
      this.POAddForm.get("address1")?.setValue(this.GetEditPurchaseOrderSummary[0].vendor_address);
      this.POAddForm.get("dispatch_mode")?.setValue(this.GetEditPurchaseOrderSummary[0].mode_despatch);
      this.POAddForm.get("template_content")?.setValue(this.GetEditPurchaseOrderSummary[0].termsandconditions);
      
      
      // this.POAddForm.get("currency_code")?.setValue(this.GetEditPurchaseOrderSummary[0].currency_code);
      this.POAddForm.get("exchange_rate")?.setValue(this.GetEditPurchaseOrderSummary[0].exchange_rate);
      this.POAddForm.get("employee_name")?.setValue(this.GetEditPurchaseOrderSummary[0].requested_by);
      this.POAddForm.get("Requestor_details")?.setValue(this.GetEditPurchaseOrderSummary[0].requested_details);
      
      this.POAddForm.get("freight_terms")?.setValue(this.GetEditPurchaseOrderSummary[0].freight_terms);
      this.POAddForm.get("totalamount")?.setValue(this.GetEditPurchaseOrderSummary[0].netamount);
      
      this.POAddForm.get("po_covernote")?.setValue(this.GetEditPurchaseOrderSummary[0].po_covernote);
      this.POAddForm.get("addoncharge")?.setValue(this.GetEditPurchaseOrderSummary[0].addon_amount);
      this.POAddForm.get("additional_discount")?.setValue(this.GetEditPurchaseOrderSummary[0].discount_amount);
      this.POAddForm.get("freightcharges")?.setValue(this.GetEditPurchaseOrderSummary[0].freightcharges);
      this.POAddForm.get("tax_name4")?.setValue(this.GetEditPurchaseOrderSummary[0].tax_name);
      this.POAddForm.get("tax_amount4")?.setValue(this.GetEditPurchaseOrderSummary[0].tax_amount);
      this.POAddForm.get("roundoff")?.setValue(this.GetEditPurchaseOrderSummary[0].roundoff);

      this.POAddForm.get("delivery_terms")?.setValue(this.GetEditPurchaseOrderSummary[0].freight_terms);
      this.POAddForm.get("po_date")?.setValue(this.GetEditPurchaseOrderSummary[0].po_date);
      this.POAddForm.get("expected_date")?.setValue(this.GetEditPurchaseOrderSummary[0].expected_date);
      this.POAddForm.get("shipping_address")?.setValue(this.GetEditPurchaseOrderSummary[0].shipping_address);
      this.POAddForm.get("payment_terms")?.setValue(this.GetEditPurchaseOrderSummary[0].payment_terms);
      this.POAddForm.get("renewal_mode")?.setValue(this.GetEditPurchaseOrderSummary[0].renewal_flag);
       this.POAddForm.get("renewal_date")?.setValue(this.GetEditPurchaseOrderSummary[0].renewal_date);
      //  this.POAddForm.get("frequency_terms")?.setValue(this.GetEditPurchaseOrderSummary[0].frequency_term);
       this.mdlfrequency_terms = this.GetEditPurchaseOrderSummary[0].frequency_term;
     
      this.POAddForm.get("grandtotal")?.setValue(this.GetEditPurchaseOrderSummary[0].total_amount);
      const emailId = this.GetEditPurchaseOrderSummary[0].email_id;
      const contactTelephonenumber = this.GetEditPurchaseOrderSummary[0].contact_telephonenumber;
      const gstNo = this.GetEditPurchaseOrderSummary[0].gst_no;
      const vendorDetails = `${emailId}\n${contactTelephonenumber}\n${gstNo}`;
      this.POAddForm.get("vendor_details")?.setValue(vendorDetails);
      this.existingFileName = this.GetEditPurchaseOrderSummary[0].file_name;
    this.existingFileUrl = this.GetEditPurchaseOrderSummary[0].file_path;
  ...
  onAnchorClick(event:any){
    ...
    var params = 
    {
      ...
      file_path:this.existingFileUrl
          ...
          this.service.post('PmrTrnPurchaseOrder/DownloadDocument',params).subscribe(
            (data: any) => {
            if (data != null){
