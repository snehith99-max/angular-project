...
@Component({
  selector: 'app-pmr-mst-taxsegment-interstate-vendor',
  templateUrl: './pmr-mst-taxsegment-interstate-vendor.component.html',
  styleUrls: ['./pmr-mst-taxsegment-interstate-vendor.component.scss']
})

export class PmrMstTaxsegmentInterstateVendorComponent {
  ...
  GetInterState(){
    var othersegmentapi = 'PmrTaxSegment/GetInterstateSegmentSummary';
    setTimeout(() => {
      $('#Getinterstate_list').DataTable();
    }, 1);
    this.service.get(othersegmentapi).subscribe((apiresponse: any) => {
      this.Getinterstate_list = apiresponse.GetPmrinterstate_list;
      setTimeout(() => {
        $('#Getinterstate_list').DataTable();
      }, 1);
      this.taxsegment_gid = apiresponse.GetPmrinterstate_list[0].taxsegment_gid;
