...
export class IunAssign {
  ...
  template_gid: string = "";
  Taxsegment2unassignvendorList:any;
  taxsegment_gid: any;
...
@Component({
  selector: 'app-pmr-trn-taxsegment2unassignvendor',
  templateUrl: './pmr-trn-taxsegment2unassignvendor.component.html',
})
export class PmrTrnTaxsegment2unassignvendorComponent {

  assignvendor_list: any[] = [];
  vendor!: Vendor;
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    ngOnInit(): void {
      ...
      var vendorassign = 'PmrTaxSegment/GetTaxSegmentAssignVendorSummary';
      this.service.getparams(vendorassign, param).subscribe((apiresponse: any) => {
        this.assignvendor_list = apiresponse.Taxsegment2assignvendorList;
        this.NgxSpinnerService.hide();
        setTimeout(() => {
          $('#assignvendor_list').DataTable();
        }, 1);
    ...
    //   const numRows = this.assignvendor_list.length;
    //   return numSelected === numRows;
    // }
    // masterToggle() {
    //   this.isAllSelected() ?
    //     this.selection.clear() :
    //     this.assignvendor_list.forEach((row: IunAssign) => this.selection.select(row));
    // }
    masterToggle() {
      const table = $('#assignvendor_list').DataTable();
      ...
      if (this.isAllSelected()) {
        ...
        filteredData.forEach((row: any) => {
          ...
          const vendor = this.assignvendor_list.find(item => item.vendor_code === vendorCode);
    ...
    unassign(){
      debugger;
        ...
        this.CurObj.Taxsegment2unassignvendorList = this.pick
        this.CurObj.taxsegment_gid=this.taxsegment_gid
          if (this.CurObj.Taxsegment2unassignvendorList.length === 0) {
        ...
        debugger
        this.NgxSpinnerService.show();
          var url = 'PmrTaxSegment/PostPmrTaxSegment2Vendor';  
