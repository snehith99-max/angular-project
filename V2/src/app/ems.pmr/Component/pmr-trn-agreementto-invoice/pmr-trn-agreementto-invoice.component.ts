...
@Component({
  selector: 'app-pmr-trn-agreementto-invoice',
  templateUrl: './pmr-trn-agreementto-invoice.component.html',
  styleUrls: ['./pmr-trn-agreementto-invoice.component.scss']
})
export class PmrTrnAgreementtoInvoiceComponent {
...
this.grandtotal = this.total_amount + (+this.tax_amount4) + (+this.addoncharge) + (+this.freightcharges) +
  ...
  GetViewPurchaseOrderSummary(renewal_gid: any) {
    debugger
    var url='PmrTrnPurchaseagreement/GetPurchaseInvoicesummary'  
    let param = {
      renewal_gid : renewal_gid
    }
    this.service.getparams(url,param).subscribe((result:any)=>{
    this.purchaseorder_list = result.GetinviocePO1;
    ...
    this.InvoiceForm.get("renewal_gid")?.setValue(this.purchaseorder_list[0].renewal_gid);
  ...
  GetViewPurchaseOrderProduct(renewal_gid: any) {
     
    var url='PmrTrnPurchaseagreement/GetPurchaseInvoiceproduct'  
    let param = {
      renewal_gid : renewal_gid 
    }
    this.service.getparams(url,param).subscribe((result:any)=>{
      this.purchaseorderproduct_list = result.GetinvioceProduct1;
      this.InvoiceForm.get("totalamount")?.setValue(result.grand_total)
  ...
  onSubmit() { 
   debugger
    var params = {
      ...
      renewal_gid: this.InvoiceForm.value.renewal_gid,
      invoice_date: this.InvoiceForm.value.invoice_date,
      renewal_date: this.InvoiceForm.value.renewal_date,
      frequency_terms: this.InvoiceForm.value.frequency_terms,
    ...
    var api6 = 'PmrTrnPurchaseagreement/PostOverAllSubmit'   
