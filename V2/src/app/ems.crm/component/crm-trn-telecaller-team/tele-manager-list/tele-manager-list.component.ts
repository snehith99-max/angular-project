...
import { TeleteamList } from './teleteam-list';
...
@Component({
	selector: 'tele-manager-list',
	templateUrl: './tele-manager-list.component.html',
	styleUrls: ['./tele-manager-list.component.scss']
})

export class TeleManagerListComponent implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'UnAssigned Managers', remove: 'Assigned Managers', all: 'All Select', none: 'All Unselect', direction: TeleManagerListComponent.LTR, draggable: true };
	...
	@Input() height = '100px';
	@Input() filter1 = false;
	@Input() format1 = TeleManagerListComponent.DEFAULT_FORMAT;
	@Input() sort1 = false;
	@Input()
	...
	available: TeleteamList;
	confirmed: TeleteamList;
	...
	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: CrmTrnTelecallerTeamComponent, private NgxSpinnerService: NgxSpinnerService,private router: Router) {
		this.available = new TeleteamList(TeleManagerListComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new TeleteamList(TeleManagerListComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: { [key2: string]: SimpleChange }) {
		...
		if (changeRecord['format1']) {
			...
			if (typeof (this.format1.direction) === 'undefined') {
				this.format1.direction = TeleManagerListComponent.LTR;
			}

			if (typeof (this.format1.add) === 'undefined') {
				this.format1.add = TeleManagerListComponent.DEFAULT_FORMAT.add;
			}

			if (typeof (this.format1.remove) === 'undefined') {
				this.format1.remove = TeleManagerListComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof (this.format1.all) === 'undefined') {
				this.format1.all = TeleManagerListComponent.DEFAULT_FORMAT.all;
			}

			if (typeof (this.format1.none) === 'undefined') {
				this.format1.none = TeleManagerListComponent.DEFAULT_FORMAT.none;
			}

			if (typeof (this.format1.draggable) === 'undefined') {
				this.format1.draggable = TeleManagerListComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	direction() {
		return this.format1.direction === TeleManagerListComponent.LTR;
	}

	dragEnd(list: TeleteamList) {
		if (list) {
	...
	drag(event: DragEvent, item: any, list: TeleteamList) {
	...
	allowDrop(event: DragEvent, list: TeleteamList) {
	...
	drop(event: DragEvent, list: TeleteamList) {
	...
	private makeUnavailable(source: TeleteamList, item: any) {
	...
	moveItem(source: TeleteamList, target: TeleteamList, item: any = null, trueup = true) {
		// console.log(pick)
	...
	btnremove(source: TeleteamList, target: TeleteamList, item: any = null, trueup = true) {

		this.CurObj.campaignassign = source.pick;

		if (this.CurObj.campaignassign.length != 0) {
			var url1 = 'CrmTeleCallerTeamSummary/PostManagerUnassignedlist'
	...
	shiftClick(event: MouseEvent, index: number, source: TeleteamList, item: any) {
	...
	selectAll(source: TeleteamList) {
		source.pick.length = 0;
		source.pick = source.sift.slice(0);
	}

	selectNone(source: TeleteamList) {
		source.pick.length = 0;
	}

	isAllSelected(source: TeleteamList) {
	...
	isAnySelected(source: TeleteamList) {
	...
	private unpick(source: TeleteamList) {
	...
	clearFilter(source: TeleteamList) {
		if (source) {
			source.picker = '';
			this.onFilter(source);
		}
	...
	addbtn(source: TeleteamList, target: TeleteamList, item: any = null, trueup = true, campaign_gid: string) {
	

		const campaignUnassignData: ICampaignAssign = { campaign_gid: campaign_gid, campaignassign: [] };
		if (target.list.length === 0) {
			campaignUnassignData.campaignassign = target.sift.map((item: any) => item._id); // Pass all values from the target list
		} else {
			campaignUnassignData.campaignassign = target.pick; // Pass selected values only
		}
		this.NgxSpinnerService.show();
		const url = 'CrmTeleCallerTeamSummary/PostManagerUnassignedlist';
	...
	GetTeamSummary() {
        ...
        this.service.get(api).subscribe((result: any) => {
            this.team_list1 = result.Telecaller_list;
