...
import {  TeleBasicList } from './telebasic-list';
...
@Component({
  selector: 'tele-dual-list',
  templateUrl: './tele-dual-list.component.html',
  styleUrls: ['./tele-dual-list.component.scss']
})

export class TeleDualListComponent implements DoCheck, OnChanges {
	...
	static DEFAULT_FORMAT = { add: 'UnAssigned Employees', remove: 'Assigned Employees', all: 'All Select', none: 'All Unselect', direction: TeleDualListComponent.LTR, draggable: true };
	...
	@Input() height = '100px';
	@Input() filter = false;
	@Input() format = TeleDualListComponent.DEFAULT_FORMAT;
	@Input() sort = false;
	@Input()
	...
	available: TeleBasicList;
	confirmed: TeleBasicList;
	...
	constructor(private differs: IterableDiffers, private ToastrService: ToastrService, public service: SocketService, public getData: CrmTrnTelecallerTeamComponent, private NgxSpinnerService: NgxSpinnerService) {
		this.available = new TeleBasicList(TeleDualListComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new TeleBasicList(TeleDualListComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
		...
		if (changeRecord['format']) {
			...
			if (typeof (this.format.direction) === 'undefined') {
				this.format.direction = TeleDualListComponent.LTR;
			}

			if (typeof (this.format.add) === 'undefined') {
				this.format.add = TeleDualListComponent.DEFAULT_FORMAT.add;
			}

			if (typeof (this.format.remove) === 'undefined') {
				this.format.remove = TeleDualListComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof (this.format.all) === 'undefined') {
				this.format.all = TeleDualListComponent.DEFAULT_FORMAT.all;
			}

			if (typeof (this.format.none) === 'undefined') {
				this.format.none = TeleDualListComponent.DEFAULT_FORMAT.none;
			}

			if (typeof (this.format.draggable) === 'undefined') {
				this.format.draggable = TeleDualListComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	direction() {
		return this.format.direction === TeleDualListComponent.LTR;
	}

	dragEnd(list: TeleBasicList) {
		if (list) {
	...
	drag(event: DragEvent, item: any, list: TeleBasicList) {
	...
	allowDrop(event: DragEvent, list: TeleBasicList) {
	...
	drop(event: DragEvent, list: TeleBasicList) {
	...
	private makeUnavailable(source: TeleBasicList, item: any) {
		const idx = source.list.indexOf(item);
		if (idx !== -1) {
			source.list.splice(idx, 1);
		}
	}
	// addbtn(source: TeleBasicList, target: TeleBasicList, item: any = null, trueup = true) {
	...
	// 			var url1 = 'CrmTeleCallerTeamSummary/PostAssignedlist'
	...
	moveItem(source: TeleBasicList, target: TeleBasicList, item: any = null, trueup = true) {
		// console.log(pick)
	...
	btnremove(source: TeleBasicList, target: TeleBasicList, item: any = null, trueup = true) {

		this.CurObj.campaignassign = source.pick;
		if (this.CurObj.campaignassign.length != 0) {
			var url1 = 'CrmTeleCallerTeamSummary/PostUnassignedlist'
			this.service.post(url1, this.CurObj).pipe().subscribe((result: any) => {
	...
	shiftClick(event: MouseEvent, index: number, source: TeleBasicList, item: any) {
	...
	selectAll(source: TeleBasicList) {
		source.pick.length = 0;
		source.pick = source.sift.slice(0);
	}

	selectNone(source: TeleBasicList) {
		source.pick.length = 0;
	}

	isAllSelected(source: TeleBasicList) {
	...
	isAnySelected(source: TeleBasicList) {
	...
	private unpick(source: TeleBasicList) {
	...
	clearFilter(source: TeleBasicList) {
		if (source) {
			source.picker = '';
			this.onFilter(source);
		}
	...
	addbtn(source: TeleBasicList, target: TeleBasicList, item: any = null, trueup = true, campaign_gid: string) {
		...
		const url = 'CrmTeleCallerTeamSummary/PostUnassignedlist';
