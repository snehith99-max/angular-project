...
@Component({
  selector: 'app-crm-smm-outlookfoldersummary',
  templateUrl: './crm-smm-outlookfoldersummary.component.html',
  styleUrls: ['./crm-smm-outlookfoldersummary.component.scss']
})
export class CrmSmmOutlookfoldersummaryComponent {
  ...
  labelid: any;
  CurObj2: IMailFolders = new IMailFolders();
  ...
  reactiveFormFoldermove!: FormGroup;
  reactiveFormFolderEdit!: FormGroup;
  ...
  ngOnInit(): void {
    this.reactiveFormFoldermove = new FormGroup({
    ...
    this.reactiveFormFolderEdit = new FormGroup({
      label_id: new FormControl(''),
      labelNameEdit: new FormControl('', [
        Validators.required,
        Validators.pattern("^(?!\\s*$)[a-zA-Z\\s]*$"),
  ...
  get label_id() {
    return this.reactiveFormFoldermove.get('label_id')!;
  }
  get labelName() {
    return this.reactiveFormFolder.get('labelName')!;
  }
  get labelNameEdit() {
    return this.reactiveFormFolderEdit.get('labelNameEdit')!;
  }
  GetFolderSummary() {
    this.NgxSpinnerService.show();
  ...
  onsubmitfolder() {
    debugger;
    // this.CurObj1.folder_name = this.reactiveFormFolder.value.folder_name;
    // this.gmailmovelist
    if (this.reactiveFormFolder.status === "VALID") {
      // console.log(this.reactiveFormFolder.value)

      this.NgxSpinnerService.show();
      const url1 = 'OutlookCampaign/OutlookCreateEmailLabel';
      this.service.post(url1, this.reactiveFormFolder.value).pipe().subscribe({
        next: (result: any) => {
  ...
  handleOutsideClick2(event2: MouseEvent) {
    if (!this.elementRef.nativeElement.contains(event2.target)) {
  ...
  GetMailSummary() {
    ...
    this.service.get(api).subscribe((result: any) => {
      ...
      if (this.folders != null && this.folders != undefined) {
        this.selectFolder(this.folders[0]);        //console.log( 'folder', this.folders[0].label_name )
        var param = {
          label_id: this.folders[0].label_id
        };
        var api1 = 'OutlookCampaign/OutlookAPIinboxFolderSummary';
        this.service.getparams(api1, param).subscribe(
          (result: any) => {
  ...
  async runBackgroundApiCall() {
    var api2 = 'OutlookCampaign/GetOutlookFolderDetails';
    try {
  ...
  selectEmail(email: Email) {
    ...
    if (this.selectedEmail.attachement_flag != 'N') {
      var url = 'OutlookCampaign/GetOutlookInboxAttchement';
      attachmentObservable = this.service.getparams(url, param);
    }

    var url1 = 'OutlookCampaign/OutlookGetReplyMail';
...
if(this.replytoid !=null && this.replytoid !="" && this.replytoid !='')
  ...
  moveToTrash() {
    ...
    if (this.CurObj.gmailmovelist.length != 0) {
      this.NgxSpinnerService.show();
      var url1 = 'OutlookCampaign/OutlookMovetoTrash'
  ...
  onclose() {
    this.reactiveFormFoldermove.reset();
    this.reactiveFormFolder.reset();
  }
  ...
  onmovefolder() {
    ...
    if (this.movetofolder_list != null && this.movetofolder_list != undefined && this.movetofolder_list.length != 0) {
      this.CurObj2.gmailmovelist = this.movetofolder_list;
      this.CurObj2.label_id = this.reactiveFormFoldermove.value.label_id;
      if (this.reactiveFormFoldermove.status == 'VALID') {
        this.NgxSpinnerService.show();
        var url1 = 'OutlookCampaign/OutlookMovetoFolder'
  ...
  editFolder(folder: any) {
    this.reactiveFormFolderEdit.get("label_id")?.setValue(folder.label_id);
    this.reactiveFormFolderEdit.get("labelNameEdit")?.setValue(folder.label_name);
    // console.log(`Edit folder: ${folder.label_name}`);
    // Add your edit folder logic here
  }
  onupdate() {
    debugger
    //this.reactiveFormFolderEdit.value;
    if (this.reactiveFormFolderEdit.status === "VALID") {
      // console.log(this.reactiveFormFolder.value)

      this.NgxSpinnerService.show();
      const url1 = 'OutlookCampaign/OutlookUpdateEmailLabel';
      this.service.post(url1, this.reactiveFormFolderEdit.value).pipe().subscribe({
        next: (result: any) => {
          if (result.status === true) {
            this.reactiveFormFolder.reset();
            this.GetMailSummary();
  ...
  deleteFolder(folder: any) {
    //console.log(`Delete folder: ${folder.label_name}`);
    this.labelid = folder.label_id;

  }
  ondelete() {
    debugger
    this.NgxSpinnerService.show();
    var url = 'OutlookCampaign/OutlookDeleteLabelOrFolder'
    let param = {
  ...
  selectFolder(folder: Folder): void {
    ...
    //console.log(this.selectedFolder.label_name)
    var param = {
      label_id: this.selectedFolder.label_id
    };
    this.NgxSpinnerService.show();
    var api1 = 'OutlookCampaign/OutlookAPIinboxFolderSummary';
    this.service.getparams(api1, param).subscribe(
