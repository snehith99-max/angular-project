...
@Component({
  selector: 'app-crm-smm-gmailtrashsummary',
  templateUrl: './crm-smm-gmailtrashsummary.component.html',
  styleUrls: ['./crm-smm-gmailtrashsummary.component.scss']
})
export class CrmSmmGmailtrashsummaryComponent {
  ...
  ngOnInit(): void {
    ...
    this.routerEventsSubscription = this.router.events.subscribe((event: any) => {
      if (event instanceof NavigationEnd) {
        ...
        if (event.url === '/crm/CrmSmmGmailTrashSummary') {
          this.NgxSpinnerService.show();
        } else {
          this.NgxSpinnerService.show();
        }
  ...
  GetMailSummary() {
    ...
    var api1 = 'GmailCampaign/GmailAPIinboxTrashSummary';
  ...
  selectEmail(email: Email) {
   ...
   truncatedHtml = this.removeFontSizeAttributes(truncatedHtml);
    ...
    if (this.selectedEmail.attachement_flag != 'N') {
      const url = 'GmailCampaign/GetInboxAttchement';
      attachmentObservable = this.service.getparams(url, param);
    }
  
    const url1 = 'GmailCampaign/GetReplyMail';
...
this.globalforwardid =slectedlist.forward_id;
  ...
  moveToInbox() {
    ...
    if (this.CurObj.gmailmovelist.length != 0) {
      this.NgxSpinnerService.show();
      var url1 = 'GmailCampaign/MoveToInbox'
  ...
  ondelete(){
    ...
    if (this.CurObj2.gmailmovelist.length != 0) {
      this.NgxSpinnerService.show();
      var url1 = 'GmailCampaign/TrashDeleteMail'
  ...
  onmovefolder() {
    ...
    if (this.movetofolder_list != null && this.movetofolder_list != undefined && this.movetofolder_list.length != 0) {
      ...
      if (this.reactiveFormFolder.status == 'VALID') {
        this.NgxSpinnerService.show();
        var url1 = 'GmailCampaign/MoveToFolderFromTrash'
