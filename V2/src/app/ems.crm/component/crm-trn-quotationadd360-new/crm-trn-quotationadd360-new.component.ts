...
import { AES, enc } from 'crypto-js';
  ...
  @Component({
    selector: 'app-crm-trn-quotationadd360-new',
    templateUrl: './crm-trn-quotationadd360-new.component.html',
    styleUrls: ['./crm-trn-quotationadd360-new.component.scss']
  })
  export class CrmTrnQuotationadd360NewComponent {
  ...
  //      this.marks = this.POProductList1[i].quantity;
  //       var discount_amount = (subtotal * this.POProductList1[i].discount_persentage) / 100;
  //       this.POProductList1[i].discount_amount = (discount_amount).toFixed(2);
  // debugger
  //        // Calculate total tax amount for the product
  //     let totalTaxAmount = 0;
  //     for (let taxSegment of this.POProductList1[i].taxSegments) {
  //         totalTaxAmount += (parseFloat(this.POProductList1[i].unitprice) * parseFloat(taxSegment.tax_percentage) / 100);
  //     }
  //     const totalTaxAmountPerQty = totalTaxAmount * this.POProductList1[i].quantity;
  //     this.POProductList1[i].totalTaxAmount = totalTaxAmountPerQty.toFixed(2);
  //     if(totalTaxAmountPerQty>0){
  //       total_amount = (totalTaxAmountPerQty+subtotal) - discount_amount;
  //      }else
  //       var total_amount = subtotal - this.POProductList1[i].discount_amount;
  //       this.POProductList1[i].total_amount = total_amount.toFixed(2);
  ...
  this.grandtotal = this.total_amount + (+this.tax_amount4) + (+this.addoncharge) + (+this.freightcharges) +
    ...
    onSubmit(){
     ...
     var params = {
      ...
      customer_name: this.QuoteAddForm.value.customer_name.customer_name,
