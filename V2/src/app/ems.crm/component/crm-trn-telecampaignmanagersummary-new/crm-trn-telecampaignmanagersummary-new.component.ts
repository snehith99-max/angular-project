...
@Component({
  selector: 'app-crm-trn-telecampaignmanagersummary-new',
  templateUrl: './crm-trn-telecampaignmanagersummary-new.component.html',
  styleUrls: ['./crm-trn-telecampaignmanagersummary-new.component.scss']
})
export class CrmTrnTelecampaignmanagersummaryNewComponent {
  responsedata: any;
 ...
 getleadbankgid(parameter: string) {
  ...
  GetTelecallerManagerTotalSummary() {
    this.NgxSpinnerService.show();
    var url = 'TelecallerManager/GetTelecallerManagerNewSummary'
 ...
 GetTeamPerformencechart() {
  ...
  GetTelecallerManagerSummary() {
    ...
    this.service.get(url).subscribe((result: any) => {
      ...
      // console.log('hello',this.countlist)
      setTimeout(() => {
        $('#countlist').DataTable();
      }, 1);
    });
