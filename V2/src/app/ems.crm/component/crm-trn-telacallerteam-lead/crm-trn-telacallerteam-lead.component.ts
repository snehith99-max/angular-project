...
@Component({
  selector: 'app-crm-trn-telacallerteam-lead',
  templateUrl: './crm-trn-telacallerteam-lead.component.html',
  styleUrls: ['./crm-trn-telacallerteam-lead.component.scss']
})
export class CrmTrnTelacallerteamLeadComponent  {
  ...
  GetUserSummary(){
    ...
    var api ='CrmTeleCallerTeamSummary/GetUserSummary'
    this.service.post(api, param).subscribe((result: any) => {
      $('#user_list').DataTable().destroy();
      this.responsedata = result;
      this.user_list = this.responsedata.detailtelacalllerteam_list1;
  ...
  GetAssignSummary() {
    ...
    var api = 'CrmTeleCallerTeamSummary/GetAssignSummary'
    this.service.get(api).subscribe((result: any) => {
      $('#assign_list').DataTable().destroy();
      this.responsedata = result;
      this.assign_list = this.responsedata.detailtelacalllerteam_list1;
      setTimeout(() => {
  ...
  OnSubmit(): void {
    ...
    this.CurObj.summary_list2 = list
    if (this.CurObj.summary_list2.length != 0) {
      this.isAnyCheckboxSelected = true;
      this.NgxSpinnerService.show();
      var url1 = 'CrmTeleCallerTeamSummary/GetAssignLead'
      this.service.post(url1,this.CurObj).pipe().subscribe((result: any) => {
        ...
        if (result.status == false) {
          ...
          this.route.navigate(['/crm/CrmTrnTelecallerTeam']);
        }
      });
    }
    else {
...
GetSortedLeads(){
  ...
  var api = 'CrmTeleCallerTeamSummary/GetSearchedLeads'
